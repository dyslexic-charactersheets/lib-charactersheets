...
var each = {
  ...
  transform: function transform(args) {
    ...
    return args.contents.flatMap(function (item) {
      ...
      if (util.isString(item) && has(args.map, item)) {
...
var sort = {
  ...
  transform: function transform(args, ctx) {
    ...
    contents = contents.sort(function (a, b) {
      ...
      if (util.isNumber(ka) || util.isNumber(kb)) return ka - kb;
...
function renderTableBasic(args, reg, doc, headings, rows) {
  ...
  if (!isNull(headings)) {
    var tcols = headings.map(function (col) {
      ...
      return "<th".concat(colCls).concat(colspan, ">").concat(elem, "</th>");
    });
...
function renderTableFlipped(args, reg, doc, headings, cols) {
  ...
  headings = headings.map(function (heading) {
    if (isNull(heading) || isString(heading) && heading == "" || util.isObject(heading) && heading.type == 'label' && isEmpty(heading.label)) {
      heading = {
        type: 'label',
        label: ''
      };
...
var table = {
  ...
  render: function render(args, reg, doc) {
    ...
    rows = rows.flatMap(function (row) {
      ...
      if (rep > 1) {
        log$1("table", "Repeating row", rep, "times:", row);
...
var field_control_checkgrid = {
  ...
  render: function render(args) {
    ...
    for (var i = 1; i <= args.max; i++) {
      ...
      var cls = elementClass("field", "control", a, [], ["control"]);
    ...
    var groups = _.chunk(checks, args.group).map(function (ch) {
      ...
      var cls = elementClass("field", "control-group", a, [], ["control", "dir", "w", "h"]);
            ...
            if (has(element, "contents")) {
              element.contents = element.contents.flatMap(function (subelement) {
                if (subelement.type == "reduce") {
                  ...
                  log$1("Document", "Reducing slots", subelement.reduce, "min = ".concat(element.min, ", max = ").concat(element.max));
                  element.min -= reduce;
                  log$1("Document", "min = ".concat(element.min, ", max = ").concat(element.max));
                  return [];
                }
