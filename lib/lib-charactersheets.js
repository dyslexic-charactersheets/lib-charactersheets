...
function () {
  ...
  _createClass(Document, [{
    ...
    value: function composeDocument(registry) {
      ...
      log("compose", " - Doc:", this.doc); // log("compose", " - Zones:", zones);
...
function parseCharacter(chardesc) {
  ...
  Object.keys(attr).forEach(function (key) {
    if (key.match(/^option/)) {
      var flag = key.replace(/^option/, '').toLowerCase(); // log("Character", "Option", flag, attr[key]);
    ...
    value: function render(callback) {
      ...
      ready(function () {
        try {
          ...
          var title = "".concat(toTitleCase(_this8.chardesc.ancestry), " ").concat(toTitleCase(_this8.chardesc.class));
          ...
          if (_this8.chardesc.favicon) {
            locateAsset(_this8.chardesc.favicon, function (logoFile) {
          ...
          document.accentColour = _this8.chardesc.accentColour; // load units
            ...
            units.forEach(function (unit) {
              if (unit.hasOwnProperty("require")) {
                unit.require.forEach(function (req) {
                  log$1("Character", "Unit ".concat(unit.id, " requires"), req);
                  if (unitIds.includes(req.unit)) return; // TODO check if the new unit is really new
