...
function getLabelHeight(args) {
  if (args === null) return 1; // args = registryDefaultArgs(args);
...
function renderHeading(h) {
  return function (args) {
    ...
    return "<".concat(h).concat(cls, ">").concat(esc(args.title), "</").concat(h, ">");
  };
}
...
var spells_list = {
  ...
  render: function (_render3) {
    ...
    for (var lvl = min; lvl <= max; lvl++) {
      ...
      if (args.special) {
        var special = interpolate(args.special, {
          'level': lvl
        });
...
var renderCompoundControl = function renderCompoundControl(args, reg) {
  ...
  var outputParts = parts.map(function (part) {
    ...
    if (_.has(part, "type") && part.type != "field") return reg.renderItem(part);
  ...
  _createClass(Document, [{
    ...
    value: function composeDocument(registry) {
      ...
      function compose(element) {
        if (!_.has(element, "type")) return [element]; // if (element.type == 'table') log("Document", "Compose item", element);
        // first recurse so we have the ingredients

        _.each(["contents", "placeholder", "header", "inputs"], function (item_key) {
          // log("compose", "Checking for", item_key);
          if (_.has(element, item_key)) {
            // log("compose", "Preparing item", item_key, element[item_key]);
            if (_.isArray(element[item_key])) element[item_key] = _.flatMap(element[item_key], compose);else element[item_key] = compose(element[item_key]);
          }
    ...
    value: function getStylesheet() {
      ...
      if (this.portraitURL) {
        ...
        cssParts.push(".portrait__inner{background-image:url('".concat(this.portraitURL, "');}"));
      } // TODO animal companion portraits


      if (this.animalURL) {
        cssParts.push(".portrait--animal .portrait__inner{background-image:url('".concat(this.animalURL, "');}"));
      } // background
    ...
    value: function renderDocument(registry) {
      // TODO load favicon
      // var faviconData = getDataURL("core", "images/"+args.favicon);
      var faviconData = this.getFavicon(); // TODO load stylesheet
    ...
    value: function create(callback) {
      ...
      ready(function () {
        try {
          ...
          var document = new Document(documentUnit); // TODO set character parameters
          ...
          document.composeDocument(_this8.registry); // write the document out for debug

          fs.writeFile(__dirname + '/../test/prototype8.json', JSON.stringify(document.doc, null, 2), function (err) {
