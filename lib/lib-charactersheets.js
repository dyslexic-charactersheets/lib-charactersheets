...
var advancement = {
  ...
  transform: function transform(args, ctx) {
    ...
    for (var _lv = 1; _lv <= args.end; _lv++) {
      var _ret = _loop(_lv);
...
function parseCharacter(primary, request) {
  ...
  switch (attr.game) {
    ...
    default:
      if (attr.class) {
        var _className = attr.class.replace(/^class[\/-]/, '');

        var _classShortName = _className.replace(/^.*\//, '');

        char.units.push('class/' + _className);
        char.classes.push(_className);

        var _classPrefix = toCamelCase('class ' + _classShortName); // log("Character", "Class name", className, ", prefix", classPrefix);


        getUnitOptions('class/' + _className);

        var _classFeatsKey = _classPrefix + 'Feats';

        if (attr[_classFeatsKey]) {
          char.classFeats = parseFeats(attr[_classFeatsKey]);
          char.classFeats.forEach(function (feat) {
            // log("Character", "Class feat:", feat);
            char.units.push('feat/' + _className + '/' + feat);
          });
        }
      }
...
loadSystemData(['common', 'pathfinder2', 'beyond-the-wall', 'premium']);
