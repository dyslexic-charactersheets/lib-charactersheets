...
var p = {
  ...
  render: function render(args, reg, doc) {
    var cls = elementClass('p', null, args, ['prose', 'blk'], {
      'align': 'left',
      'size': 'medium'
    ...
    return "<p".concat(cls, ">").concat(icon).concat(_e(args.content, doc), "</p>");
  }
};
var portrait = {
  name: 'portrait',
  ...
  _createClass(Document, [{
    ...
    value: function getStylesheet() {
      ...
      this.units.forEach(function (unit) {
        ...
        if (unit.id != "document") rendered = replaceColours(rendered, _this8.printColour, _this8.accentColour, _this8.highContrast);
    ...
    value: function render(callback) {
      ...
      ready(function () {
        try {
          ...
          document.language = _this10.data.language; // TODO get title parts from inside units
          // TODO translate title parts

          var titleParts = [];

          if (_this10.data.ancestry) {
            titleParts.push(toTitleCase(_this10.data.ancestry));
          }

          _this10.data.classes.forEach(function (cls) {
            titleParts.push(toTitleCase(cls));
          });

          _this10.data.archetypes.forEach(function (archetype) {
            titleParts.push(toTitleCase(archetype));
          });

          var title = titleParts.join(" ");

          if (_this10.data.name) {
            title = "".concat(_this10.data.name, ", ").concat(title);
          }
...
function addTranslator$1(callback) {
