      ...
      ready(function () {
        try {
          var _ret = function () {
            ...
            document.language = _this9.data.language; // TODO get title parts from inside units
            ...
            document.watermark = _this9.data.printWatermark; // get known vars from the data

            knownVars.forEach(function (varname) {
              if (has(_this9.data, varname)) {
                var key = toKebabCase(varname);
                var value = _this9.data[varname]; // log("Character", `Var: ${key} = ${JSON.stringify(value)}`);
