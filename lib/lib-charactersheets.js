          ...
          try {
            ...
            knownVars.forEach(function (varname) {
              if (has(data, varname)) {
                ...
                var value = data[varname]; // log("Character", `Var: ${key} = ${JSON.stringify(value)}`);
            ...
            self.loadQueue.ready(function () {
              Events.createElementTreeEvt.call(document.doc, document.title, self.request); // render the document
...
function (_Instance2) {
  ...
  function Party(primary, request, registry) {
    ...
    _this11 = _possibleConstructorReturn(this, _getPrototypeOf(Party).call(this));
    _this11.registry = registry;
    _this11.data = parseParty(primary);
    var characterDefaults = {
      game: _this11.data.game,
      language: _this11.data.language
    };
    _this11.members = _this11.data.members.map(function (chardesc) {
  ...
  _createClass(Party, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this12.members.forEach(function (member) {
...
function () {
  ...
  _createClass(Build, [{
    ...
    value: function render(callback) {
      ...
      ready(function () {
        try {
          log$1("Build", "Data", _this13.data);
          var system = getSystem(_this13.data.game);
          var documentUnit = system.getUnit("document");
          var document = new Document(documentUnit);
          document.language = _this13.data.language;
          var units = system.getUnits(_this13.data.units);
          units = system.inferUnits(units);
          ...
          document.composeDocument(_this13.registry);
          document.title = "Build a character"; // log("Build", "Document", document);
          // render the document

          var data = document.renderDocument(_this13.registry);
          callback({
            data: data,
  ...
  _createClass(Request, [{
    ...
    value: function parse(request) {
      ...
      if (has(request, "included")) {
        if (isArray(request.included)) {
          request.included.forEach(function (instance) {
            return _this14.addInstance(instance, false);
          });
        }
      ...
      if (isArray(request.data)) {
        request.data.forEach(function (instance) {
          return _this14.addInstance(instance, true);
        });
      } else {
    ...
    value: function getPrimaries(registry) {
        ...
        if (has(primary, "relationships")) {
          Object.keys(primary.relationships).forEach(function (relkey) {
            primary.attributes[relkey] = primary.relationships[relkey].data.flatMap(function (link) {
              if (has(_this15.instances, link.id)) {
                return [_this15.instances[link.id]];
              } else {
                return [];
              }
      ...
      return primaries.map(function (primary) {
        switch (primary.type) {
          case 'character':
            return new Character(primary, _this15, registry);

          case 'party':
            return new Party(primary, _this15, registry);

          case 'quick':
            switch (primary.attributes['quick']) {
              case 'build':
                return new Build(primary, _this15, registry);
            }
