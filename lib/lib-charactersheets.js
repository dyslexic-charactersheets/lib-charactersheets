...
var level_marker = {
  ...
  render: function render(args, reg, doc) {
    ...
    var marker = args.marker ? "<label>".concat(__(args.marker), "</label>") : '';
    return "<div class='level-marker'>".concat(marker, "<div class='level-marker__level'>").concat(__(level), "</div></div>");
  }
};
var list = {
  name: 'list',
...
function () {
  ...
  _createClass(Registry, [{
    ...
    value: function renderItem(item, doc) {
      ...
      if (util.isString(item.exists)) item.exists = item.exists.replace(/#{.*?}/g, '');
...
function () {
  ...
  _createClass(Document, [{
    ...
    value: function composeDocument(registry) {
      ...
      var self = this; // the lesser form: only expand zones within, don't do full unit expansion
    ...
    value: function getJavascript() {
      var jsParts = []; // jsParts.push("alert('Yo.');");
      ...
      ready(function () {
        try {
          ...
          if (_this9.data.printBackground) {
            var printBackground = _this9.data.printBackground;
