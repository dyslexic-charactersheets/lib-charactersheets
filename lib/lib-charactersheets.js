...
var article = {
  ...
  render: function render(args, reg, doc) {
    ...
    var header = "<header>".concat(reg.render(headerElements, doc), "</header>");
    ...
    if (_.isFunction(args.template)) {
      ...
      rowCallback = function rowCallback(row) {
        ...
        return row.map(function (cell) {
          ...
          return "<td>".concat(reg.renderItem(cell, doc), "</td>");
        });
      };
    }
...
var renderCompoundControl = function renderCompoundControl(args, reg, doc) {
  ...
  var outputParts = parts.map(function (part) {
    ...
    i++;
    if (part.hasOwnProperty("type") && part.type != "field") return reg.renderItem(part, doc);
...
function () {
  ...
  _createClass(System, [{
    ...
    value: function getUnits(codes) {
      ...
      var units = codes.map(function (code) {
...
function () {
  ...
  _createClass(Document, [{
    ...
    value: function getVar(varname) {
      if (!this.vars.hasOwnProperty(varname)) return false; // TODO combine multiple values somehow
      ...
      if (unit.hasOwnProperty("inc")) {
        unit.inc.forEach(function (include) {
          ...
          if (include.hasOwnProperty("set")) {
            if (!_this5.vars.hasOwnProperty(include.set)) _this5.vars[include.set] = [];
  ...
  switch (attr.game) {
    case 'pathfinder2':
      ...
      if (attr.class) {
        ...
        var cls = toCamelCase('class ' + char.class);
        var classFeatsKey = cls + 'Feats';
        ...
        Object.keys(attr).forEach(function (key) {
          ...
          if (key.startsWith(cls) && !key.endsWith('Feats')) {
            var selname = key.replace(cls, ''); // log("Character", "Class option", key, selname, attr[key]);
...
function () {
  function Character(primary, request, registry) {
    ...
    this.chardesc = parseCharacter(primary);
  }
  ...
  _createClass(Character, [{
    ...
    value: function render(callback) {
      ...
      var self = this; // log("Character", "Render character");
      ...
      ready(function () {
        try {
          ...
          if (_this8.chardesc.ancestry) {
            titleParts.push(toTitleCase(_this8.chardesc.ancestry));
          }

          _this8.chardesc.classes.forEach(function (cls) {
            titleParts.push(toTitleCase(cls));
          });

          _this8.chardesc.archetypes.forEach(function (archetype) {
            titleParts.push(toTitleCase(archetype));
          });

          var title = titleParts.join(" ");

          if (_this8.chardesc.name) {
            title = "".concat(_this8.chardesc.name, ", ").concat(title);
          }
          ...
          if (_this8.chardesc.favicon) {
            locateAsset(_this8.chardesc.favicon, function (faviconFile) {
              loadQueue.loadEmbed(faviconFile).then(function (data) {
                document.faviconURL = toDataURL(data, _this8.chardesc.favicon);
              });
            });
          }

          if (_this8.chardesc.printLogo) {
            locateAsset(_this8.chardesc.printLogo, function (logoFile) {
              loadQueue.loadEmbed(logoFile).then(function (data) {
                document.logoURL = toDataURL(data, _this8.chardesc.printLogo);
              });
            });
          }

          if (_this8.chardesc.printPortrait) {
            locateAsset(_this8.chardesc.printPortrait, function (portraitFile) {
              loadQueue.loadEmbed(portraitFile).then(function (data) {
                document.portraitURL = toDataURL(data, _this8.chardesc.printPortrait);
              });
            });
          }

          if (_this8.chardesc.animalPortrait) {
            locateAsset(_this8.chardesc.animalPortrait, function (animalPortraitFile) {
              loadQueue.loadEmbed(animalPortraitFile).then(function (data) {
                document.animalURL = toDataURL(data, _this8.chardesc.animalPortrait);
              });
            });
          }

          if (_this8.chardesc.printBackground) {
            locateAsset(_this8.chardesc.printBackground, function (backgroundFile) {
              loadQueue.loadEmbed(backgroundFile).then(function (data) {
                document.backgroundURL = toDataURL(data, _this8.chardesc.printBackground);
              });
            });
          } // TODO set character parameters


          document.printColour = _this8.chardesc.printColour;
          document.accentColour = _this8.chardesc.accentColour;
          document.watermark = _this8.chardesc.printWatermark; // load units

          var units = system.getUnits(self.chardesc.units); // infer required units (to a finite depth)
          ...
          loadQueue.ready(function () {
            ...
            Events.createElementTreeEvt.call(document.doc, title, _this8.request); // fs.writeFile(__dirname + '/../test/out/test.json', JSON.stringify(document.doc, null, 2), (err) => {
            //   if (err)
...
function () {
  function Party(primary, request, registry) {
    ...
    this.registry = registry;
    this.partydesc = parseParty(primary);
  }
...
function () {
  ...
  _createClass(Request, [{
    ...
    value: function parse(request) {
      ...
      if (Object.hasOwnProperty(request, "included")) {
        if (Array.isArray(request.included)) {
          request.included.forEach(function (instance) {
            return _this9.addInstance(instance, false);
          });
        }
      ...
      if (Array.isArray(request.data)) {
        request.data.forEach(function (instance) {
          return _this9.addInstance(instance, true);
        });
      } else {
    ...
    value: function addInstance(instance, primary) {
      if (!Object.hasOwnProperty(instance, "id")) instance.id = randomID();
    ...
    value: function getPrimaries(registry) {
      ...
      this.primary.forEach(function (primary) {
        ...
        if (Object.hasOwnProperty(primary, "relationships")) {
          Object.keys(primary.relationships).forEach(function (relkey) {
            primary.attributes[relkey] = primary.relationships[relkey].data.flatMap(function (link) {
              if (Object.hasOwnProperty(_this10.instances, link.id)) {
                return [_this10.instances[link.id]];
              } else {
                return [];
              }
            });
            log$1("Request", "Substituted relationships: ", relkey, "=", primary.attributes[relkey]);
          });
        }
      });
      return this.primary.map(function (primary) {
        switch (primary.type) {
          case 'character':
            return new Character(primary, _this10, registry);

          case 'party':
            return new Party(primary, _this10, registry);
