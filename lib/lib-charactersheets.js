      ...
      if (attr.class) {
        ...
        Object.keys(attr).forEach(function (key) {
          ...
          if (key.startsWith(classPrefix) && !key.endsWith('Feats')) {
            var selname = toKebabCase(key.replace(classPrefix, '')); // log("Character", "Class selector", selname);

            if (isArray(attr[key])) {
              attr[key].forEach(function (selvalue) {
                selvalue = toKebabCase(selvalue); // log("Character", "Class option", key, selname, "=", selvalue);

                var unitname = classShortName + '/' + selname + '/' + selvalue; // log("Character", "Subclass unit name", unitname);
              ...
              var unitname = classShortName + '/' + selname + '/' + selvalue; // log("Character", "Class option unit", unitname);
      ...
      if (attr.archetypes && isArray(attr.archetypes)) {
        attr.archetypes.forEach(function (archetype) {
          if (isString(archetype)) {
            ...
            char.units.push('archetype/' + archetype); // log("Character", "Archetype:", "archetype/"+archetype);
          }
        });
      }
