  ...
  _createClass(Instance, [{
    ...
    value: function getAsset(asset, callback) {
      ...
      if (isObject$1(asset) && has(asset, "id") && !has(asset, "data")) {
        ...
        log("Instance", "known instances", Object.keys(this.data.instances)); // log("Instance", "all known instances", this.data.instances);
...
function parseCharacter(primary, request) {
  ...
  log("Character", "Request attributes", attr); // an object to start with
  ...
  switch (attr.game) {
    ...
    case STARFINDER_2:
      ...
      if (attr.classes) {
        var _iterator20 = _createForOfIteratorHelper(attr.classes),
            _step20;

        try {
          var _loop5 = function _loop5() {
            var cls = _step20.value;
      ...
      var _iterator21 = _createForOfIteratorHelper(archetypes),
          _step21;

      try {
        for (_iterator21.s(); !(_step21 = _iterator21.n()).done;) {
          var archetype = _step21.value;
    ...
    case 'core-fantasy':
      ...
      if (attr.class) {
        var className = attr.class.replace(/^class[\/-]/, ''); // let classShortName = className.replace(/^.*\//, '');
        ...
        promise.then(function (data) {
          ...
          ready(function () {
            try {
              ...
              document.setMeasurementUnits(data.measurementUnits); // log("Character", "Doc measurement units", document.measurementUnits);
...
var Mini = /*#__PURE__*/function (_Instance5) {
  ...
  function Mini(primary, request, registry) {
    ...
    _this24.registry = registry;
    _this24.request = request;
    _this24.promise = new Promise(function (resolve, reject) {
      ready(function () {
        _this24.data = parseMini(primary, request);
        _this24.data.units = [].concat(_toConsumableArray(_this24.data.units), _toConsumableArray(_this24.getDataUnits(_this24.data.isLoggedIn, _this24.data.isNoCalc)));
        resolve(_this24.data);
      });
    });
    return _this24;
  }
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        promise.then(function (data) {
          ...
          log("Mini", "Units: ".concat(_this25.data.units));
          ready(function () {
            try {
              var system = getSystemStack(data.game);
              ...
              var documentUnit = system.getUnit("document");
              var document = new Document(system, documentUnit, data.id);
              document.request = _this25.request; // language
              ...
              document.accentColour = data.accentColour;
              document.watermark = data.printWatermark;
              log("Mini", "Document colour", document.printColour); // get known vars from the data

              knownVars$2.forEach(function (varname) {
                if (has(data, varname)) {
                  var key = toKebabCase(varname);
                  var _value10 = data[varname]; // log("Character", `Var: ${key} = ${JSON.stringify(value)}`);

                  document.setVar(key, _value10, "high");
                }
              }); // load units
              ...
              document.summary = __("Map Mini"); // make the element tree
  ...
  _createClass(Build, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        ready(function () {
          try {
            log("Build", "Data", _this26.data);
            var system = getSystemStack(_this26.data.game);
            var documentUnit = system.getUnit("document");
            var document = new Document(system, documentUnit);
            document.language = _this26.data.language;
            var units = system.getUnits(_this26.data.units);
            units = system.inferUnits(units);
            ...
            document.composeDocument(_this26.registry);
            document.title = "Build a character"; // log("Build", "Document", document);
            // render the document

            var data = document.renderDocument(_this26.registry);
            resolve({
              data: data,
              filename: document.title + ".html",
              mimeType: "text/html"
...
var Custom = /*#__PURE__*/function (_Instance6) {
  ...
  function Custom(primary, request, registry) {
    ...
    _this27.registry = registry;
    _this27.request = request;
    _this27.promise = new Promise(function (resolve, reject) {
      ready(function () {
        _this27.request = request;
        _this27.data = parseCustom(primary);
        resolve(_this27.data);
      });
    });
    return _this27;
  }
  ...
  _createClass(Custom, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        promise.then(function (data) {
          log("Custom", "Units: ".concat(_this28.data.units));
          ready(function () {
            try {
              ...
              var documentUnit = system.getUnit("document");
              var document = new Document(system, documentUnit, data.id);
              document.request = _this28.request; // language
  ...
  _createClass(Request, [{
    ...
    value: function parse(request) {
      ...
      if (has(request, "included")) {
        if (isArray(request.included)) {
          request.included.forEach(function (instance) {
            return _this29.addInstance(instance, false);
          });
        }
      ...
      if (isArray(request.data)) {
        request.data.forEach(function (instance) {
          return _this29.addInstance(instance, true);
        });
      } else {
    ...
    value: function getPrimaries(registry) {
        ...
        if (has(primary, "relationships")) {
          Object.keys(primary.relationships).forEach(function (relkey) {
            primary.attributes[relkey] = primary.relationships[relkey].data.flatMap(function (link) {
              if (has(_this30.instances, link.id)) {
                return [_this30.instances[link.id]];
              } else {
                return [];
              }
      ...
      return primaries.map(function (primary) {
        ...
        switch (primary.type) {
          case 'character':
            return new Character(primary, _this30, registry);

          case 'party':
            return new Party(primary, _this30, registry);

          case 'kingmaker':
          case 'kingdom':
            return new Kingdom(primary, _this30, registry);

          case 'gm':
            switch (primary.attributes['gm']) {
              case 'characters':
                return new GM_Party(primary, _this30, registry);

              case 'maps':
                return new GM_Maps(primary, _this30, registry);
          ...
          case 'mini':
            return new Mini(primary, _this30, registry);

          case 'quick':
            switch (primary.attributes['quick']) {
              case 'build':
                return new Build(primary, _this30, registry);
            }

          case 'custom':
            return new Custom(primary, _this30, registry);
