        ...
        units.forEach(function (unit) {
          ...
          if (has(unit, "require")) {
            unit.require.forEach(function (req) {
              log("System", "Unit ".concat(unit.id, " requires"), req); // check if the new unit is really new
...
function () {
  ...
  _createClass(Document, [{
    ...
    value: function renderDocument(registry) {
      log("Document", "Pages", this.doc.contents.map(function (page) {
        return "".concat(page.id, ": ").concat(page.name);
      }));
