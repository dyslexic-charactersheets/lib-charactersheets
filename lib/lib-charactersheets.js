...
function spellLevel(lvl, ord, style, slots, special, special_value) {
  ...
  if (special) {
    if (isString(special)) {
      ...
      special = spellField(lvl, style, "special", special, special_value); // log("spells", "Special", special);
    }
...
var spells_list = {
  ...
  transform: function transform(args, ctx) {
    ...
    for (var _lvl2 = min; _lvl2 <= max; _lvl2++) {
      ...
      spell_levels.push(spellLevel(_lvl2, ord, args.style, slots[_lvl2], args.special, special_value));
    }
  ...
  _createClass(GM_Party, [{
    ...
    value: function completeDocument(document) {
      ...
      this.data.colors.forEach(function (color, i) {
        log("GM", "Adding color", i, color); // TODO custom Sass compilation for color blocks
