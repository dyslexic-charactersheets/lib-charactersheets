...
function replaceColours(str, documentColour, accentColour, highContrast) {
  str = str.replace(/#[0-9a-fA-F]{6}/g, function (c) {
    return adjustColour(c, documentColour, highContrast);
  });
...
function adjustColour(c, documentColour, highContrast) {
  try {
    var base = color(c);
...
function __(str, doc) {
  ...
  return str.replace(/_\{([\s\S]*?)\}/g, function (m, p) {
    return translate(p);
  });
}
...
var article = {
  ...
  render: function render(args, reg, doc) {
    ...
    var annotation = args.annotation ? "<div class='article__annotation'>".concat(__(args.annotation), "</div>") : '';
...
var calc = {
  ...
  render: function render(args, reg, doc) {
    ...
    args.calc = true;
    var cls = elementClass('row', null, args, ["calc", "inline", "labelHeight", "pad"], {
      'layout': 'center'
...
var g = {
  ...
  render: function render(args, reg, doc) {
    var cls = elementClass('g', null, args, ['pad'], {
      'galign': 'justify',
      'valign': 'center'
    });
...
function renderHeading(h) {
  return function (args, reg, doc) {
    ...
    return "<".concat(h).concat(cls, ">").concat(esc(__(args.title), true), "</").concat(h, ">");
  };
}
...
var class_name = {
  ...
  render: function render(args, reg, doc) {
    var preface = isEmpty(args.preface) ? '' : "<h5>".concat(esc(__(args.preface), true), "</h5>");
    var name = "<h2>".concat(esc(__(args.title), true), "</h2>");
    var affix = isEmpty(args.affix) ? '' : "<h5>".concat(esc(__(args.affix), true), "</h5>");
    var cls = elementClass('class-name', null, args, [], {
      flex: 'medium'
    });
    return "<div".concat(cls, ">").concat(preface).concat(name).concat(affix).concat(reg.render(args.contents, doc), "</div>");
  }
};
var hr = {
  name: 'hr',
...
var label = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "<label".concat(cls, ">").concat(esc(__(args.label), true), "</label>");
  }
};
var layout = {
  name: 'layout',
...
var level_marker = {
  ...
  render: function render(args, reg, doc) {
    ...
    var marker = args.marker ? "<label>".concat(esc(__(args.marker), true), "</label>") : '';
    return "<div class='level-marker'>".concat(marker, "<div class='level-marker__level'>").concat(esc(__(level)), "</div></div>");
  }
};
var list = {
  name: 'list',
  ...
  render: function render(args, reg, doc) {
    ...
    var cls = elementClass('list', null, args, ["zebra", "zebra-inverse", "collapse", "vr", "hr", "light", "merge-bottom"], {
      "flex": false
    });
    return "<div".concat(cls, ">").concat(reg.render(args.contents, doc), "</div>");
  },
...
var p = {
  ...
  render: function render(args, reg, doc) {
    var cls = elementClass('p', null, args, ['prose', 'pad'], {
      'align': 'left',
      'size': 'medium'
    ...
    return "<p".concat(cls, ">").concat(icon).concat(esc(__(args.content), true), "</p>");
  }
};
var ul = {
  name: 'ul',
...
var li = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "<li>".concat(reg.render(args.contents, doc), "</li>");
  }
};
var dl = {
  name: 'dl',
  ...
  render: function render(args, reg, doc) {
    var defs = Object.keys(args.defs).map(function (term) {
      ...
      return "<div><dt>".concat(esc(__(term)), "</dt><dd>").concat(esc(__(termdef)), "</dd></div> ");
    });
    var dlCls = elementClass('dl', null, args, ['div']);
...
var portrait = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "<figure".concat(cls, "><div class='portrait__inner'></div><figcaption>").concat(__(copyright), "</figcaption></figure>");
  }
};
var proficiency = {
  name: 'proficiency',
...
var row = {
  ...
  render: function render(args, reg, doc) {
    ...
    var cls = elementClass('row', null, args, ['unlabelled', 'narrow', 'pad'], {
      'layout': 'left',
      'valign': 'bottom',
      'lp': '',
      'rb': '',
...
var ruby = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "<div".concat(cls, "><label class='ruby__text'>").concat(esc(__(args.ruby)), "</label>").concat(reg.render(args.contents, doc), "</div>");
  }
};
var section = {
  name: 'section',
  ...
  render: function render(args, reg, doc) {
    ...
    var title = args.untitled ? '' : "<h3>".concat(esc(__(args.title)), "</h3>");
...
var span = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "<span".concat(cls, ">").concat(esc(__(args.content), true), "</span>");
  }
};
...
var field = {
  ...
  render: function render(args, reg, doc) {
    ...
    var cls = elementClass('field', null, args, ["icon", "ref", "misc", "temp", "indent"], {
      "frame": "normal",
      "width": "medium",
      "align": "centre",
      "size": "medium",
...
function fieldDefaults(args, reg, doc) {
  ...
  if (args.frame == "none" || args.frame == "annotation") {
    args.lp = 0; // log("field", "Frameless, no label padding");
  } else {
    args.lp = getLabelHeight(args); // log("field", `Frame ${args.frame}, label padding ${args.lp}`);
  }
...
function fieldInner(args, reg, doc) {
  ...
  var unit = has(args, "unit") && !!args.unit ? "<label class='field__unit'>".concat(__(args.unit), "</label>") : '';
  ...
  if (args.repeat > 1) {
    ...
    for (var i = 0; i < args.repeat; i++) {
      ...
      if (i == args.repeat - 1 && args['merge-bottom'] && boxargs['border'] == 'bottom') boxargs['border'] = 'none';
    ...
    if (args['merge-bottom'] && boxargs['border'] == 'bottom') boxargs['border'] = 'none';
...
function defaultFrameRender(args, reg, doc) {
  ...
  var label = args.label ? "<label".concat(ident.for, ">").concat(esc(__(args.label), true), "</label>") : '';
  var legend = args.legend ? "<legend>".concat(esc(__(args.legend), true), "</legend>") : '';
  return "".concat(legend).concat(label).concat(fieldInner(args, reg, doc));
...
var field_frame_right = {
  ...
  render: function render(args, reg, doc) {
    ...
    var label = args.label ? "<label".concat(ident.for, ">").concat(esc(__(args.label), true), "</label>") : '';
    var legend = args.legend ? "<legend>".concat(esc(__(args.legend), true), "</legend>") : '';
    return "".concat(fieldInner(args, reg, doc)).concat(legend).concat(label);
  }
};
var field_frame_h_label = {
  ...
  render: function render(args, reg, doc) {
    ...
    var label = args.label ? "<label class='field__h-label'".concat(ident.for, ">").concat(esc(__(args.label), true), "</label>") : ''; // WRONG! The h-label is supposed to go inside the box!

    return "".concat(label).concat(fieldInner(args, reg, doc));
  }
};
...
var field_frame_annotation = {
  ...
  render: function render(args, reg, doc) {
    var ident = fieldIdent(args.id);
    var label = args.label ? "<label class='field__annotation'".concat(ident.for, ">").concat(esc(__(args.label), true), "</label>") : '';
    return "".concat(label).concat(fieldInner(args, reg, doc));
  }
};
...
function defaultControlRender(args, reg, doc) {
  ...
  var value = args.value == '' ? '' : " value='".concat(__(args.value), "'");
  var attr = args.editable ? '' : 'readonly';
  var input = "<input".concat(ident.ident).concat(value).concat(attr, ">");
  var underlay = args.underlay ? "<u>".concat(__(args.underlay), "</u>") : '';
  var prefix = args.prefix ? "<span class='field__overlay'>".concat(__(args.prefix), "</span>") : '';
  var suffix = args.suffix ? "<span class='field__overlay'>".concat(__(args.suffix), "</span>") : '';
...
var field_control_value = {
  ...
  render: function render(args, reg, doc) {
    ...
    var suffix = args.suffix ? "<span class='field__overlay'>".concat(__(args.suffix), "</span>") : '';
    var underlay = args.underlay ? "<u>".concat(__(args.underlay), "</u>") : '';
    var value = "<span>".concat(esc(__(args.value)), "</span>");
...
var field_control_alignment = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "\n      <i class='field__grid'></i>\n      <i class='icon icon_good'></i>\n      <i class='icon icon_evil'></i>\n      <i class='icon icon_lawful'></i>\n      <i class='icon icon_chaotic'></i>\n\n      <label class='field__good'>".concat(__("Good"), "</label>\n      <label class='field__evil'>").concat(__("Evil"), "</label>\n      <label class='field__lawful'>").concat(__("Lawful"), "</label>\n      <label class='field__chaotic'>").concat(__("Chaotic"), "</label>\n\n      ").concat(radios.join(""), "\n    ");
  }
};
...
var field_control_money = {
  ...
  render: function render(args, reg, doc) {
    ...
    var suffix = "<span class='field__overlay'>".concat(__(unit), "</span>");
    var ident = fieldIdent(args.id);
    var cls = elementClass("field", "control", args, [], {
      "digits": 0,
      "shift": 0,
...
function () {
  function Registry() {
    ...
    [unit, // document,
    advancement, article, blockquote, calc, class_name, class_icon, each, g, h1, h2, h3, h4, h5, h6, hr, tail, vr, icon, label, layout, indent, level, level_marker, list, join, logelem, logo, nothing, page, p, ul, li, dl, portrait, proficiency, action, selectable, repeat, row, ruby, section, sort, slots, spacer, span, spells_list, spells_table, spells_list2, table, copy, paste, unit, zone, field, field_frame_none, field_frame_above, field_frame_left, field_frame_right, field_frame_h_label, field_frame_annotation, field_frame_circle, field_control_input, field_control_value, field_control_speed, field_control_alignment, field_control_boost, field_control_checkbox, field_control_radio, field_control_checkgrid, field_control_compound, field_control_progression, field_control_money, field_control_weight, field_control_proficiency, field_control_proficiency_icon, field_control_icon].forEach(function (elem) {
  ...
  _createClass(Document, [{
        ...
        switch (element.type) {
          ...
          case 'slots':
            ...
            if (reg && reg.transform) {
              ...
              var args = Object.assign({}, reg.defaults, element); //const ctx = { zones: self.zones, templates: self.templates, largePrint: self.largePrint, locale: self.language };
    ...
    value: function getStylesheet() {
      ...
      this.units.forEach(function (unit) {
        ...
        if (unit.id != "document") rendered = replaceColours(rendered, _this7.printColour, _this7.accentColour, _this7.highContrast);
      ...
      ready(function () {
        try {
          var _ret = function () {
            ...
            if (_this9.data.printLarge) {
              document.largePrint = true;
            }

            if (_this9.data.printHighContrast) {
            ...
            if (_this9.data.printBackground) {
              var printBackground = _this9.data.printBackground; // log("Character", "Background:", printBackground);
              ...
              if (has(bgColours, printBackground)) {
                ...
                _this9.getAsset(printBackground, function (dataURL) {
