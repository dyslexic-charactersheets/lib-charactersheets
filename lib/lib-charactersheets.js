...
var sort = {
  ...
  transform: function transform(args, ctx) {
    ...
    if (args.group) {
      ...
      var _iterator2 = _createForOfIteratorHelper(groupkeys),
          _step2;
      ...
      var _iterator5 = _createForOfIteratorHelper(groupkeys),
          _step5;

      try {
        for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
          var _group3 = _step5.value;

          var _iterator6 = _createForOfIteratorHelper(itemsbygroup[_group3]),
              _step6;
...
var spells_list = {
  ...
  transform: function transform(args, ctx) {
    ...
    for (var _lvl2 = min; _lvl2 <= max; _lvl2++) {
      ...
      if (args.special && isArray(args["special-value"]) && args["special-value"].length > _lvl2 - 1) {
        special_value = args["special-value"][_lvl2 - 1];
      }
...
function fieldInner(args, reg, doc, decoration) {
  ...
  if (args.repeat > 1) {
    ...
    for (var i = 0; i < args.repeat; i++) {
      var _value4 = i >= values.length ? null : values[i];
    ...
    for (var _i7 = 0; _i7 < args.parts.length; _i7++) {
      var _value5 = _i7 >= _values2.length ? null : _values2[_i7];
      ...
      var _box2 = "<div class='field__boxes__inner' data-subid='".concat(partArgs.id, "'><div").concat(_boxcls, ">").concat(_i7 == 0 ? icon : '').concat(_control).concat(partUnit, "</div>").concat(_ruby2, "</div>");
          ...
          if (has(unit, "require")) {
            unit.require.forEach(function (req) {
              ...
              if (!isNull(newunit)) {
                ...
                log("Character", "Required unit not found: ".concat(req.unit));
              }
            });
          }
  ...
  _createClass(Instance, [{
    ...
    value: function getDataUnits(isLoggedIn, isNoCalc) {
      if (isLoggedIn && !isNoCalc) {
        return ["data/edit", "data/calc", "data/save" // "data/note",
        // "data/roll",
        ];
      } else {
        return ["data/no-edit"];
...
var Document = /*#__PURE__*/function () {
  ...
  _createClass(Document, [{
    ...
    value: function getVar(varname) {
      ...
      var _iterator7 = _createForOfIteratorHelper(this.vars[varname]),
          _step7;

      try {
        for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
          var _include = _step7.value;
      ...
      var _iterator8 = _createForOfIteratorHelper(incs),
          _step8;

      try {
        for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
          var _include2 = _step8.value;
      ...
      if (is_array) {
        ...
        var _iterator9 = _createForOfIteratorHelper(incs),
            _step9;

        try {
          for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
            var include = _step9.value;
    ...
    value: function addUnit(unit) {
      ...
      if (has(unit, "inc")) {
        var _iterator10 = _createForOfIteratorHelper(unit.inc),
            _step10;

        try {
          for (_iterator10.s(); !(_step10 = _iterator10.n()).done;) {
            var include = _step10.value;
      ...
      function pushCalculation(id, eq) {
        ...
        if (has(calcFields, id)) {
          if (!forgiving && calcFields[id] != eq) {
            error$1("Document", "Duplicate calculation ".concat(id.yellow, ":"), calcFields[id], eq);
          }
        } else {
          calcFields[id] = eq;
          fields.push({
            ...
            if (!isString(element.eq)) {
              ...
              switch (element.control) {
                case 'speed':
                  ...
                  var affix = element.units == 'metric' ? '--m' : '--ft'; // log("Document", "Speed eq", element.eq);
    ...
    value: function renderDocument(registry) {
      ...
      var isLoggedIn = this.isLoggedIn; // the actual content
...
function parseCharacter(primary, request) {
  ...
  log("Character", "Is no calc?", char.isNoCalc, attr.browserTarget); // log("Character", "Request attributes", attr);
        ...
        promise.then(function (data) {
          ...
          ready(function () {
            try {
              ...
              document.request = _this13.request; // language
              ...
              knownVars.forEach(function (varname) {
                if (has(data, varname)) {
                  ...
                  var _value6 = data[varname]; // log("Character", `Var: ${key} = ${JSON.stringify(value)}`);

                  document.setVar(key, _value6, "high");
                }
              ...
              var _title2 = __("Character");

              var _summary = __("Character");

              switch (system.code) {
                case 'pathfinder2':
                  _title2 = pathfinder2Title(units, document, data, true);
                  _summary = pathfinder2Title(units, document, data, false);
                  break;
              }

              document.title = _title2;
              document.summary = _summary; // make the element tree
...
var Kingdom = /*#__PURE__*/function (_Instance3) {
  ...
  function Kingdom(primary, request, registry) {
    ...
    _this16.promise = new Promise(function (resolve, reject) {
      ready(function () {
        _this16.data = parseKingdom(primary);
...
var GM_Instance = /*#__PURE__*/function (_Instance4) {
  ...
  _createClass(GM_Instance, [{
    ...
    value: function parseGM_Instance(primary, request) {
      ...
      this.data.isNoCalc = attr.browserTarget == "pdf", log("GM", "Is logged in?", attr.isLoggedIn, char.isLoggedIn);
  ...
  _createClass(Build, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        ready(function () {
          try {
            log("Build", "Data", _this22.data);
            var system = getSystem(_this22.data.game);
            var documentUnit = system.getUnit("document");
            var document = new Document(documentUnit);
            document.language = _this22.data.language;
            var units = system.getUnits(_this22.data.units);
            ...
            document.composeDocument(_this22.registry);
            document.title = "Build a character"; // log("Build", "Document", document);
            // render the document

            var data = document.renderDocument(_this22.registry);
            resolve({
              data: data,
              filename: document.title + ".html",
              mimeType: "text/html"
...
var Custom = /*#__PURE__*/function (_Instance5) {
  ...
  function Custom(primary, request, registry) {
    ...
    _this23.registry = registry;
    _this23.request = request;
    _this23.promise = new Promise(function (resolve, reject) {
      ready(function () {
        _this23.request = request;
        _this23.data = parseCustom(primary);
        resolve(_this23.data);
      });
    });
    return _this23;
  }
  ...
  _createClass(Custom, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        promise.then(function (data) {
          log("Custom", "Units: ".concat(_this24.data.units));
          ready(function () {
            try {
              ...
              var documentUnit = system.getUnit("document");
              var document = new Document(documentUnit, data.id);
              document.request = _this24.request; // language
              ...
              var _title3 = data.name;
              document.title = _title3; // make the element tree
  ...
  _createClass(Request, [{
    ...
    value: function parse(request) {
      ...
      if (has(request, "included")) {
        if (isArray(request.included)) {
          request.included.forEach(function (instance) {
            return _this25.addInstance(instance, false);
          });
        }
      ...
      if (isArray(request.data)) {
        request.data.forEach(function (instance) {
          return _this25.addInstance(instance, true);
        });
      } else {
    ...
    value: function getPrimaries(registry) {
        ...
        if (has(primary, "relationships")) {
          Object.keys(primary.relationships).forEach(function (relkey) {
            primary.attributes[relkey] = primary.relationships[relkey].data.flatMap(function (link) {
              if (has(_this26.instances, link.id)) {
                return [_this26.instances[link.id]];
              } else {
                return [];
              }
      ...
      return primaries.map(function (primary) {
        ...
        switch (primary.type) {
          case 'character':
            return new Character(primary, _this26, registry);

          case 'party':
            return new Party(primary, _this26, registry);

          case 'kingdom':
            return new Kingdom(primary, _this26, registry);

          case 'gm':
            switch (primary.attributes['gm']) {
              case 'characters':
                return new GM_Party(primary, _this26, registry);

              case 'maps':
                return new GM_Maps(primary, _this26, registry);
          ...
          case 'quick':
            switch (primary.attributes['quick']) {
              case 'build':
                return new Build(primary, _this26, registry);
            }

          case 'custom':
            return new Custom(primary, _this26, registry);
