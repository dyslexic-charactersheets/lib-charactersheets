  ...
  transform: function transform(args, ctx) {
    ...
    var _loop = function _loop(_lv) {
      ...
      rows.push(Object.assign(flags, {
        level: _lv,
        advance: labels.join(", ") // proficiencies: proficiencies
...
var section = {
  ...
  render: function render(args, reg, doc) {
    ...
    var title = args.untitled ? '' : "<h3>".concat(esc(__(args.title)), "</h3>"); // var content = (args.contents.length == 1 && args.contents[0].type == "g") ? render(args.contents) : `<div class='g'>${render(args.contents)}</div>`;
...
function spellField(lvl, style, n, annotation) {
  ...
  var frame = "none";
  var label = null; // var border = "bottom";
...
var defaultControlRender = function defaultControlRender(args, reg, doc) {
  args = Object.assign({
    align: "center",
    width: "medium",
    editable: true,
...
function () {
  function System(system) {
    ...
    system.units.forEach(function (unit) {
      ...
      if (has(unit, "form")) {
        unit.form.forEach(function (formitem) {
          var key = Object.keys(formitem)[0]; // log("System", `Found form item ${key}`, formitem);
        });
      }
    });
  }
...
function () {
  ...
  _createClass(Document, [{
    ...
    value: function composeDocument(registry) {
      ...
      var self = this; // the lesser form: only expand a few types, don't do full unit expansion
