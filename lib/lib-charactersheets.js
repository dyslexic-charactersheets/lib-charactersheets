...
function embed(contents) {
  ...
  if (isObject$1(contents) && has(contents, "type")) {
    return contents;
  }
...
function elementClass(block) {
  ...
  var cls = [];
  var prefix = isNull(element) ? block : "".concat(block, "__").concat(element);
...
var advancement = {
  ...
  transform: function transform(args, ctx) {
    var table_id = isEmpty$1(args.id) ? 'advancement' : args.id;
        ...
        if (!isEmpty$1(proficiencyAdvances[tier])) {
          proficiencyItems.push({
            ...
            contents: [{
              ...
              type: "g",
              contents: proficiencyAdvances[tier].map(function (item) {
...
function adjustColour(c, documentColour, intensity, highContrast) {
  try {
    ...
    error$1('util', 'adjustColour: Colour error:', x, x.stack);
    return c;
  }
}
...
var dl = {
  ...
  render: function render(args, reg, doc) {
    var defs = Object.keys(args.defs).map(function (term) {
      ...
      if (args.min) term = "";
      if (icon !== "") icon = "<i class='icon icon_".concat(icon, " size_small'></i>");
...
var each = {
  ...
  transform: function transform(args) {
    ...
    return args.contents.flatMap(function (item) {
      ...
      if (i < args.rows.length && isObject$1(args.rows[i])) values = _objectSpread(_objectSpread({}, args.rows[i]), values);
...
var list = {
  ...
  transform: function transform(args) {
    ...
    if (args.columns > 1) {
      ...
      var footer = isEmpty$1(args.contents) ? [] : args.footer;
      ...
      if (args.flowv) {
        ...
        var _split = Math.ceil((args.contents.length + 0.0) / (args.columns + 0.0));
    ...
    if (args.flatten) {
      ...
      args.contents = args.contents.flatMap(function (item) {
        if (isObject$1(item) && has(item, "type") && item.type == "list") {
          flattened = true;
...
var p = {
  ...
  render: function render(args, reg, doc) {
    ...
    var title = args.title != '' ? "<span class='p__title'>".concat(_e(args.title, doc), "</span> ") : '';
...
var sort = {
  ...
  transform: function transform(args, ctx) {
    ...
    if (args.group) {
      ...
      log("sort", "Group by ".concat(args.group), groupkeys);
...
var slots = {
  ...
  transform: function transform(args, ctx, reg) {
    var debug = args.title != "";
    if (debug) {
      warn$1("slots", "SLOTS", args.title);
    ...
    if (args.even && contents.length % 2 != 0) {
      ...
      if (debug) log("slots", "Balance slot", _values);
...
function renderTableFlipped(args, reg, doc, headings, cols) {
  ...
  headings = headings.map(function (heading) {
    if (isNull(heading) || isString(heading) && heading == "" || isObject$1(heading) && heading.type == 'label' && isEmpty$1(heading.label)) {
      heading = {
        type: 'label',
        label: ''
      };
  ...
  transform: function transform(args, ctx) {
    ...
    if (isArray(args.template) && args.template.length > 0) {
      var templateCells = args.template.flatMap(function (cell) {
        if (isObject$1(cell) && has(cell, "type") && cell.type == "calc") {
    ...
    if (args.colourful) {
      ...
      try {
        for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
          ...
          var _iterator0 = _createForOfIteratorHelper(_row2.cells),
            _step0;
          try {
...
function weigh(item) {
  ...
  if (isArray(item)) {
    var weight = 0;
    var _iterator10 = _createForOfIteratorHelper(item),
      _step10;
    try {
...
var field_frame_h_label = {
  ...
  render: function render(args, reg, doc) {
    ...
    var label = args.label ? "<label class='field__h-label'".concat(ident.for, ">").concat(_e(args.label, doc), "</label>") : '';
...
var field_control_radio = {
  ...
  render: function render(args, reg, doc) {
    ...
    var selected = args.value && doc.hasVar(args.id) && args.value == doc.getVar(args.id);
    var checked = selected ? ' checked' : '';
    return "<div".concat(cls, "><input type='radio'").concat(checked).concat(ident.ident, "><label").concat(ident.for, "></label></div>");
  }
};
...
var SystemStack = /*#__PURE__*/function () {
  ...
  _createClass(SystemStack, [{
    key: "getUnit",
    value: function getUnit(code) {
      var _iterator11 = _createForOfIteratorHelper(this.systems),
        _step11;
      try {
        for (_iterator11.s(); !(_step11 = _iterator11.n()).done;) {
          var system = _step11.value;
...
function getSystemStack(code) {
  ...
  stack = [].concat(_toConsumableArray(stack), [PREMIUM, COMMON]);
  ...
  // make systems
  var systems = [];
  var _iterator12 = _createForOfIteratorHelper(stack),
    _step12;
  try {
    for (_iterator12.s(); !(_step12 = _iterator12.n()).done;) {
      var systemCode = _step12.value;
...
var MIME_WOFF = 'application/x-font-woff;charset=utf-8';
...
var Instance = /*#__PURE__*/function () {
  ...
  _createClass(Instance, [{
    ...
    value: function getAsset(asset, callback) {
      ...
      if (isObject$1(asset) && has(asset, "id") && !has(asset, "data")) {
        log("Instance", "getAsset: looking for", asset);
        log("Instance", "known instances", Object.keys(this.data.instances));
        // log("Instance", "all known instances", this.data.instances);
        if (has(this.data.instances, asset.id)) {
          ...
          if (!has(asset, "type") || asset.type == instance.type) {
            // log("Instance", "getAsset: known instance (obj) correct type", asset);
            asset = instance;
          } else {
            warn$1("Instance", "getAsset: known instance (obj) but wrong type: %s != %s", asset.type, instance.type, asset);
          }
        } else {
          error$1("Instance", "getAsset: Asset not found", asset);
          return;
        }
      }
      if (asset === null) {
    ...
    value: function getDataUnits(isLoggedIn, isNoCalc) {
      if (isLoggedIn && !isNoCalc) {
        return ["data/edit", "data/calc", "data/save",
        // "data/search",
        // "data/note",
        // "data/roll",
        "document/zoom"
  ...
  _createClass(Document, [{
    ...
    value: function getVar(varname) {
      ...
      var high = [],
        medium = [],
        low = [];
      var _iterator13 = _createForOfIteratorHelper(this.vars[varname]),
        _step13;
      try {
        for (_iterator13.s(); !(_step13 = _iterator13.n()).done;) {
          var _include = _step13.value;
      ...
      var incs = isEmpty$1(high) ? isEmpty$1(medium) ? low : medium : high;
      // log("Document", `get var '${varname}': ${JSON.stringify(incs)}`);
      if (isEmpty$1(incs)) return false;

      // TODO type hints

      // TODO combine multiple values somehow
      var is_array = false;
      var _iterator14 = _createForOfIteratorHelper(incs),
        _step14;
      try {
        for (_iterator14.s(); !(_step14 = _iterator14.n()).done;) {
          var _include2 = _step14.value;
      ...
      if (is_array) {
        var values = [];
        var _iterator15 = _createForOfIteratorHelper(incs),
          _step15;
        try {
          for (_iterator15.s(); !(_step15 = _iterator15.n()).done;) {
            var include = _step15.value;
    ...
    value: function addUnit(unit) {
      ...
      if (has(unit, "inc")) {
        var _iterator16 = _createForOfIteratorHelper(unit.inc),
          _step16;
        try {
          for (_iterator16.s(); !(_step16 = _iterator16.n()).done;) {
            var include = _step16.value;
    ...
    value: function addAtZone(zoneId, elements, replace) {
      ...
      try {
        for (_iterator17.s(); !(_step17 = _iterator17.n()).done;) {
          ...
          if (replace && isObject$1(element)) element.replace = true;
    ...
    value: function composeElement(element, registry) {
      ...
      if (isArray(element)) {
        ...
        return element.flatMap(function (e) {
          return _this0.composeElement(e, registry);
        });
      }
      if (isString(element)) {
        return [element];
      ...
      if (!has(element, "type")) {
        if (has(element, "cells")) {
          ...
          element.cells = element.cells.flatMap(function (e) {
            return _this0.completeElement(e, registry);
          });
      ...
      for (var _i0 = 0, _arr = ["contents", "placeholder", "header", "inputs", "parts"]; _i0 < _arr.length; _i0++) {
        var item_key = _arr[_i0];
        ...
        if (has(element, item_key)) {
          ...
          if (isArray(element[item_key])) element[item_key] = element[item_key].flatMap(function (el) {
            return _this0.composeElement(el, registry);
      ...
      while (this.delayedBlocks.length > 0) {
        ...
        blocks.forEach(function (block) {
          ...
          if (has(block, "params") || has(template, "params")) {
            var params = _objectSpread(_objectSpread({}, template.params), block.params);
            // log("Document", "Interpolating parameters", params);
            contents = interpolate(contents, params, _this1);
          }
            ...
            if (!isString(element.eq)) {
              ...
              switch (element.control) {
                case 'speed':
                  ...
                  var affix = doc.measurementUnits == 'metric' ? '--m' : '--ft';
          ...
          if (has(element, "parts")) {
            element.parts.forEach(function (part) {
              if (has(part, "eq") && has(part, "subid")) {
                var partid = part.subid == "" ? element.id : element.id + '-' + part.subid;
    ...
    value: function getStylesheet() {
      ...
      this.units.forEach(function (unit) {
        ...
        if (unit.id != "document") {
          ...
          rendered = replaceColours(rendered, _this10.printColour, _this10.accentColour, _this10.printIntensity, _this10.highContrast);
        }
    ...
    value: function renderDocument(registry) {
      ...
      var showSpotlight = this.hasUnit('document/spotlight');

      // the actual content
      var mainContent = registry.render(this.doc.contents, this);
...
function parseCharacter(primary, request) {
  ...
  Object.keys(attr).forEach(function (key) {
    ...
    if (flag.match(/^option-/)) {
      ...
      var ok = char.options[option] = !!attr[key];
  ...
  switch (attr.game) {
    ...
    case STARFINDER_2:
      ...
      if (attr.classes) {
        var _iterator18 = _createForOfIteratorHelper(attr.classes),
          _step18;
        try {
          var _loop5 = function _loop5() {
            var cls = _step18.value;
            var className = cls.replace(/^class[\/-]/, '');
            var classShortName = className.replace(/^.*\//, '');
            char.units.push('class/' + className);
            char.classes.push(className);
            var classPrefix = toCamelCase('class ' + classShortName);
      ...
      var _iterator19 = _createForOfIteratorHelper(archetypes),
        _step19;
      try {
        for (_iterator19.s(); !(_step19 = _iterator19.n()).done;) {
          var archetype = _step19.value;
    ...
    case 'core-fantasy':
      if (attr.lineage) {
        var lineageName = attr.lineage.replace(/^lineage[\/-]/, '');
        // let classShortName = className.replace(/^.*\//, '');
        char.units.push('lineage/' + lineageName);
      }
      if (attr.heritage) {
        var heritageName = attr.heritage.replace(/^heritage[\/-]/, '');
        // let classShortName = className.replace(/^.*\//, '');
        char.units.push('heritage/' + heritageName);
      }
      if (attr.background) {
        var backgroundName = attr.background.replace(/^background[\/-]/, '');
        // let classShortName = className.replace(/^.*\//, '');
...
var Character = /*#__PURE__*/function (_Instance) {
  ...
  function Character(primary, request, registry) {
    ...
    _this12.promise = new Promise(function (resolve, reject) {
      ready(function () {
        _this12.data = parseCharacter(primary, request);
        _this12.data.units = [].concat(_toConsumableArray(_this12.data.units), _toConsumableArray(_this12.getDataUnits(_this12.data.isLoggedIn, _this12.data.isNoCalc)));
        resolve(_this12.data);
      });
    });
    return _this12;
  }
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        promise.then(function (data) {
          ...
          log("Character", "Units: ".concat(_this13.data.units));
          ready(function () {
            try {
              ...
              document.request = _this13.request;
              ...
              if (document.measurementUnits == "metric") {
                _this13.data.units.push("core/metric");
              }
              ...
              if (data.printPortrait) {
                self.getAsset(data.printPortrait, function (dataURL, custom, path) {
                  ...
                  if (custom) {
                    ...
                    warn$1("Character", "Portrait copyright?", path);
...
var Party = /*#__PURE__*/function (_Instance2) {
  ...
  function Party(primary, request, registry) {
    ...
    _this14.data = parseParty(primary);
    var characterDefaults = {
      game: _this14.data.game,
      language: _this14.data.language
    };
    _this14.members = _this14.data.members.map(function (chardesc) {
  ...
  _createClass(Party, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this15.members.forEach(function (member) {
...
var Kingdom = /*#__PURE__*/function (_Instance3) {
  ...
  function Kingdom(primary, request, registry) {
    ...
    _this16.promise = new Promise(function (resolve, reject) {
      ready(function () {
        _this16.data = parseKingdom(primary, request);
        _this16.data.units = [].concat(_toConsumableArray(_this16.data.units), _toConsumableArray(_this16.getDataUnits(_this16.data.isLoggedIn, _this16.data.isNoCalc)));
        resolve(_this16.data);
      });
    });
    return _this16;
  }
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        promise.then(function (data) {
          // log("Character", "Render character");
          // log("Character", `Name: ${this.data.name}, game: ${this.data.game}`);
          log("Character", "Units: ".concat(_this17.data.units));
          ready(function () {
            try {
              var system = getSystemStack(data.game);
              if (system === null) {
              ...
              var documentUnit = system.getUnit("document");
              var document = new Document(system, documentUnit, data.id);
              document.request = _this17.request;
...
var GM_Instance = /*#__PURE__*/function (_Instance4) {
  ...
  _createClass(GM_Instance, [{
    ...
    value: function parseGM_Instance(primary, request) {
      ...
      this.data.isNoCalc = attr.browserTarget == "pdf", log("GM", "Is logged in?", request.isLoggedIn, this.data.isLoggedIn);
      log("GM", "Is no calc?", this.data.isNoCalc, attr.browserTarget);
      this.data.units = [].concat(_toConsumableArray(this.data.units), _toConsumableArray(this.getDataUnits(request.isLoggedIn, this.isNoCalc)));
    }
  }, {
    key: "render",
    value: function render() {
      ...
      var data = this.data;
      return new Promise(function (resolve, reject) {
        log("GM", "Units: ".concat(_this19.data.units));
        ready(function () {
          try {
            var system = getSystemStack(data.game);
            if (system === null) {
...
var GM_Party = /*#__PURE__*/function (_GM_Instance) {
  ...
  function GM_Party(primary, request, registry) {
    ...
    _this20 = _super5.call(this, request, registry);
    _this20.data = parseGM_Party(primary, request);
    _this20.parseGM_Instance(primary, request);
    return _this20;
  }
...
var GM_Maps = /*#__PURE__*/function (_GM_Instance2) {
  ...
  function GM_Maps(primary, request, registry) {
    ...
    _this21 = _super6.call(this, request, registry);
    _this21.data = parseGM_Maps(primary, request);
    _this21.parseGM_Instance(primary, request);
    return _this21;
  }
...
function parseMini(primary, request) {
  ...
  // included assets
  ["printPortrait", "printBackground"].forEach(function (field) {
    if (attr[field]) {
      var id = attr[field];
      if (isObject$1(id)) {
        id = id.id;
...
var Mini = /*#__PURE__*/function (_Instance5) {
  _inherits(Mini, _Instance5);
  var _super7 = _createSuper(Mini);
  function Mini(primary, request, registry) {
    var _this22;
    _classCallCheck(this, Mini);
    _this22 = _super7.call(this);
    _this22.registry = registry;
    _this22.request = request;
    _this22.promise = new Promise(function (resolve, reject) {
      ready(function () {
        _this22.data = parseMini(primary, request);
        _this22.data.units = [].concat(_toConsumableArray(_this22.data.units), _toConsumableArray(_this22.getDataUnits(_this22.data.isLoggedIn, _this22.data.isNoCalc)));
        resolve(_this22.data);
      });
    });
    return _this22;
  }
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        promise.then(function (data) {
          ...
          log("Mini", "Units: ".concat(_this23.data.units));
          ready(function () {
            try {
              var system = getSystemStack(data.game);
              if (system === null) {
                error$1("Mini", "System not found:", data.game);
                reject();
                return;
              }
              ...
              var documentUnit = system.getUnit("document");
              var document = new Document(system, documentUnit, data.id);
              document.request = _this23.request;
              ...
              if (data.printPortrait) {
                ...
                self.getAsset(data.printPortrait, function (dataURL, custom, path) {
                  ...
                  if (custom) {
                    ...
                    warn$1("Mini", "Portrait copyright?", path);
                    document.portraitCopyright = "paizo";
                  }
                });
              }
              ...
              // get known vars from the data
              knownVars$2.forEach(function (varname) {
                if (has(data, varname)) {
                  var key = toKebabCase(varname);
                  var _value0 = data[varname];
                  // log("Character", `Var: ${key} = ${JSON.stringify(value)}`);
                  document.setVar(key, _value0, "high");
                }
              });
...
var Custom = /*#__PURE__*/function (_Instance6) {
  _inherits(Custom, _Instance6);
  var _super8 = _createSuper(Custom);
        ...
        promise.then(function (data) {
          ...
          ready(function () {
            try {
              ...
              var documentUnit = system.getUnit("document");
              var document = new Document(system, documentUnit, data.id);
              document.request = _this26.request;
...
var Request = /*#__PURE__*/function () {
  ...
  _createClass(Request, [{
    ...
    value: function getPrimaries(registry) {
      ...
      primaries = primaries.map(function (primary) {
        ...
        switch (primary.type) {
          ...
          case 'gm':
            switch (primary.attributes['gm']) {
              ...
              case 'kingmaker':
                return new Kingdom(primary, _this28, registry);
              default:
                error$1("Request", "No valid primary in GM request");
                return null;
            }
          case 'mini':
            return new Mini(primary, _this28, registry);
          case 'quick':
          ...
          default:
            error$1("Request", "No valid primary in request");
            return null;
        }
      });
      log("Request", "getPrimaries: decoded", primaries.length, "primaries");
      primaries = primaries.filter(function (p) {
        return p != null;
      });
...
loadSystemData([COMMON, PATHFINDER_2, PATHFINDER_2_REMASTER,
// STARFINDER_2,
// CORE_FANTASY,
PREMIUM]);
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      if (primaries.length == 0) {
        error$1("lib", "No primary", chardesc);
        reject("No primary");
        return;
      }
