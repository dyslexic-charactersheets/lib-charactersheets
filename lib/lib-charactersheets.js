...
function fieldInner(args, reg, doc, decoration) {
  ...
  if (args.repeat > 1) {
    ...
    for (var i = 0; i < args.repeat; i++) {
      var _value5 = i >= values.length ? null : values[i];
    ...
    for (var _i7 = 0; _i7 < args.parts.length; _i7++) {
      var _value6 = _i7 >= _values2.length ? null : _values2[_i7];
...
function replaceColours(str, documentColour) {
  ...
  str = str.replace(/#[0-9a-fA-F]{6}/g, function (c) {
    return adjustColour$1(c, documentColour, intensity, highContrast);
  });
  str = str.replace(/%23[0-9a-fA-F]{6}/g, function (c) {
    c = c.replace('%23', '#');
    c = adjustColour$1(c, documentColour, intensity, highContrast);
    c = c.replace('#', '%23');
    return c;
  });
...
function adjustColour$1(c, documentColour, intensity, highContrast) {
  try {
    c = c.trim();
    if (c == "") c = "#808080";
  ...
  _createClass(Document, [{
      ...
      function pushCalculation(id, eq) {
        ...
        if (has(calcFields, id)) {
          if (!forgiving && calcFields[id] != eq) {
            error$1("Document", "Duplicate calculation ".concat(id, ":"), calcFields[id], eq);
          }
        } else {
          calcFields[id] = eq;
          fields.push({
    ...
    value: function getStylesheet() {
      ...
      this.units.forEach(function (unit) {
        ...
        if (unit.id != "document") {
          log("Document", "Replacing unit colours", unit.id, _this10.printColour, _this10.accentColour);
...
function parseCharacter(primary, request) {
  ...
  if (isEmpty$1(char.accentColour)) {
    char.accentColour = adjustColour$1('#707070', char.printColour, char.printIntensity);
  } // get all the flags
            ...
            try {
              ...
              knownVars.forEach(function (varname) {
                if (has(data, varname)) {
                  ...
                  var _value7 = data[varname]; // log("Character", `Var: ${key} = ${JSON.stringify(value)}`);

                  document.setVar(key, _value7, "high");
                }
        ...
        promise.then(function (data) {
          ...
          ready(function () {
            try {
              ...
              var documentUnit = system.getUnit("document");
              var document = new Document(documentUnit, data.id);
              document.request = _this17.request; // language
...
function parseGM_Party(primary, request) {
  ...
  if (isEmpty$1(gm.accentColour)) {
    gm.accentColour = adjustColour$1('#707070', gm.printColour, gm.printIntensity);
  } // accessibility options
