...
var p = {
  ...
  render: function render(args, reg, doc) {
    var cls = elementClass('p', null, args, ['blk'], {
      'align': 'left',
      'size': 'medium'
...
var ul = {
  ...
  render: function render(args, reg, doc) {
    var cls = elementClass('ul', null, args, ['blk']);
...
function () {
  ...
  _createClass(System, [{
    ...
    key: "inferUnits",
    value: function inferUnits(units, system) {
      var _this5 = this;
    ...
    value: function render(callback) {
      ...
      ready(function () {
        try {
          ...
          log$1("Character", "Language", document.language);
          ...
          units = system.inferUnits(units); // log("Character", "Units:", units.map(unit => unit.id));
          ...
          _this10.loadQueue.ready(function () {
            // log("Character", "Ready");
            Events.createElementTreeEvt.call(document.doc, document.title, _this10.request); // fs.writeFile(__dirname + '/../test/out/test.json', JSON.stringify(document.doc, null, 2), (err) => {
            //   if (err)
