...
var collate_pages = {
  ...
  transform: function transform(args, doc, reg) {
    ...
    pages = duplicatePages(pages, doc); // Combine half-pages into a single page
...
function duplicatePages(pages, doc) {
  if (isEmpty$1(doc.request)) {
    warn("collate-pages", "Request is unset", doc);
    return pages;
  }

  var attr = doc.request.primary[0].attributes; // log("collate-pages", "Page counts", attr);

  var numInventory = has(attr, "numInventory") ? 0 + attr.numInventory : 0;
  var numSpellbook = has(attr, "numSpellbook") ? 0 + attr.numSpellbook : 0;
  var numFormulaBook = has(attr, "numFormulaBook") ? 0 + attr.numFormulaBook : 0;
  var out = [];

  function addDuplicates(page, count, offset) {
    for (var i = 0; i < count; i++) {
      var number = i + offset + 1;
      var thispage = cloneDeep(page);
      thispage.id = "".concat(page.id, "/").concat(number);
      thispage = interpolate(thispage, {
        pagenum: number
      }, doc);
      out.push(thispage);
    }
  ...
  try {
    for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
      ...
      switch (_page.id) {
        ...
        case "spellbook":
          addDuplicates(_page, numSpellbook - pastSpellbook, pastSpellbook);
          break;

        case "formula-book":
          addDuplicates(_page, numFormulaBook, 0);
          break;
...
function combinePages(pages) {
  ...
  for (var i = 0; i < pages.length; i++) {
    ...
    if (has(_page2, "half") && _page2.half) {
      // log("page", "Collate: half page", page.id);
      var nextPage = pages[i + 1];
      var id = _page2.id;
      var name = _page2.name;
      var replacement = [embed(_page2.contents)];
      ...
      var pageNumbered = !has(_page2, 'numbered') || _page2.numbered;
      var nextPageNumbered = !has(_page2, 'numbered') || nextPage.numbered;
      out.push({
        type: 'page',
        order: _page2.order,
        id: id,
...
var sort = {
  ...
  transform: function transform(args, ctx) {
    ...
    if (args.group) {
      ...
      var _iterator6 = _createForOfIteratorHelper(contents),
          _step6;
      ...
      var _iterator8 = _createForOfIteratorHelper(groupkeys),
          _step8;

      try {
        for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
          var _group3 = _step8.value;

          var _iterator9 = _createForOfIteratorHelper(itemsbygroup[_group3]),
              _step9;
...
var spells_list = {
  ...
  transform: function transform(args, ctx, reg) {
    ...
    for (var _lvl2 = min; _lvl2 <= max; _lvl2++) {
      ...
      if (specialValues.length >= _lvl2) {
        special_value = specialValues[_lvl2 - min];
      }
    ...
    if (args.colourful) {
      ...
      try {
        for (_iterator10.s(); !(_step10 = _iterator10.n()).done;) {
          var _row2 = _step10.value;

          var _iterator11 = _createForOfIteratorHelper(_row2.cells),
              _step11;
...
function weigh(item) {
  ...
  if (isArray(item)) {
    ...
    var _iterator13 = _createForOfIteratorHelper(item),
        _step13;
...
var SystemStack = /*#__PURE__*/function () {
  ...
  _createClass(SystemStack, [{
    key: "getUnit",
    value: function getUnit(code) {
      var _iterator14 = _createForOfIteratorHelper(this.systems),
          _step14;

      try {
        for (_iterator14.s(); !(_step14 = _iterator14.n()).done;) {
          var system = _step14.value;
          ...
          if (has(unit, "require")) {
            unit.require.forEach(function (req) {
              ...
              if (!isNull(newunit)) {
                ...
                log("SystemStack", "Required unit not found: ".concat(req.unit));
              }
            });
          }
        }); // log("Character", `Infer units: #${i} = ${more}`);
...
function getSystemStack(code) {
  ...
  var _iterator15 = _createForOfIteratorHelper(stack),
      _step15;

  try {
    for (_iterator15.s(); !(_step15 = _iterator15.n()).done;) {
      var systemCode = _step15.value;
...
var Document = /*#__PURE__*/function () {
  ...
  _createClass(Document, [{
    ...
    value: function getVar(varname) {
      ...
      var _iterator16 = _createForOfIteratorHelper(this.vars[varname]),
          _step16;

      try {
        for (_iterator16.s(); !(_step16 = _iterator16.n()).done;) {
          var _include = _step16.value;
      ...
      if (is_array) {
        ...
        var _iterator18 = _createForOfIteratorHelper(incs),
            _step18;
    ...
    value: function addUnit(unit) {
      ...
      if (has(unit, "inc")) {
        var _iterator19 = _createForOfIteratorHelper(unit.inc),
            _step19;

        try {
          for (_iterator19.s(); !(_step19 = _iterator19.n()).done;) {
            var include = _step19.value;
    ...
    value: function addAtZone(zoneId, elements, replace) {
      ...
      var _iterator20 = _createForOfIteratorHelper(elements),
          _step20;
    ...
    value: function renderDocument(registry) {
      ...
      var showZoom = this.hasUnit('document/zoom'); // the actual content
