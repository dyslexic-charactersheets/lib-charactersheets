      ...
      ready(function () {
        try {
          ...
          if (_this8.chardesc.printLogo) {
            loadQueue.loadEmbed(__dirname + '/assets/' + _this8.chardesc.printLogo).then(function (data) {
              document.logoURL = toDataURL(data, _this8.chardesc.printLogo);
            });
          }

          if (_this8.chardesc.printPortrait) {
            loadQueue.loadEmbed(__dirname + '/assets/' + _this8.chardesc.printPortrait).then(function (data) {
              document.portraitURL = toDataURL(data, _this8.chardesc.printPortrait);
            });
          }

          if (_this8.chardesc.animalPortrait) {
            loadQueue.loadEmbed(__dirname + '/assets/' + _this8.chardesc.animalPortrait).then(function (data) {
              document.animalURL = toDataURL(data, _this8.chardesc.animalPortrait);
            });
          }

          if (_this8.chardesc.printBackground) {
            loadQueue.loadEmbed(__dirname + '/assets/' + _this8.chardesc.printBackground).then(function (data) {
          ...
          var units = system.getUnits(self.chardesc.units); // required units

          units.forEach(function (unit) {
            if (unit.hasOwnProperty("require")) {
              log$1("Character", "Unit ".concat(unit.name, " requires"), unit.require);
          ...
          loadQueue.ready(function () {
            ...
            fs$1.writeFile(__dirname + '/../test/test.json', JSON.stringify(document.doc, null, 2), function (err) {
