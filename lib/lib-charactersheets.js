...
function () {
  function Character(primary, request, registry) {
    ...
    this.data = parseCharacter(primary);
  }
  ...
  _createClass(Character, [{
    ...
    value: function render(callback) {
      ...
      ready(function () {
        try {
          var system = getSystem(_this8.data.game);

          if (system === null) {
            error$1("Character", "System not found:", _this8.data.game);
            return;
          ...
          if (_this8.data.ancestry) {
            titleParts.push(toTitleCase(_this8.data.ancestry));
          }

          _this8.data.classes.forEach(function (cls) {
            titleParts.push(toTitleCase(cls));
          });

          _this8.data.archetypes.forEach(function (archetype) {
          ...
          if (_this8.data.name) {
            title = "".concat(_this8.data.name, ", ").concat(title);
          }
          ...
          if (_this8.data.favicon) {
            locateAsset(_this8.data.favicon, function (faviconFile) {
              loadQueue.loadEmbed(faviconFile).then(function (data) {
                document.faviconURL = toDataURL(data, _this8.data.favicon);
              });
            });
          }

          if (_this8.data.printLogo) {
            locateAsset(_this8.data.printLogo, function (logoFile) {
              loadQueue.loadEmbed(logoFile).then(function (data) {
                document.logoURL = toDataURL(data, _this8.data.printLogo);
              });
            });
          }

          if (_this8.data.printPortrait) {
            locateAsset(_this8.data.printPortrait, function (portraitFile) {
              loadQueue.loadEmbed(portraitFile).then(function (data) {
                document.portraitURL = toDataURL(data, _this8.data.printPortrait);
              });
            });
          }

          if (_this8.data.animalPortrait) {
            locateAsset(_this8.data.animalPortrait, function (animalPortraitFile) {
              loadQueue.loadEmbed(animalPortraitFile).then(function (data) {
                document.animalURL = toDataURL(data, _this8.data.animalPortrait);
              });
            });
          }

          if (_this8.data.printBackground) {
            locateAsset(_this8.data.printBackground, function (backgroundFile) {
              loadQueue.loadEmbed(backgroundFile).then(function (data) {
                document.backgroundURL = toDataURL(data, _this8.data.printBackground);
              });
            });
          } // TODO set character parameters


          document.printColour = _this8.data.printColour;
          document.accentColour = _this8.data.accentColour;
          document.watermark = _this8.data.printWatermark; // load units

          var units = system.getUnits(_this8.data.units); // infer required units (to a finite depth)
          ...
          loadQueue.ready(function () {
            ...
            Events.createElementTreeEvt.call(document.doc, document.title, _this8.request); // fs.writeFile(__dirname + '/../test/out/test.json', JSON.stringify(document.doc, null, 2), (err) => {
            //   if (err)
...
function () {
  function Party(primary, request, registry) {
    ...
    log$1("Party", "Members:", this.members);
  }
...
function () {
  ...
  _createClass(Request, [{
    ...
    value: function addInstance(instance, primary) {
      if (!instance.hasOwnProperty("id")) instance.id = randomID();
        ...
        if (primary.hasOwnProperty("relationships")) {
          Object.keys(primary.relationships).forEach(function (relkey) {
            primary.attributes[relkey] = primary.relationships[relkey].data.flatMap(function (link) {
              if (_this10.instances.hasOwnProperty(link.id)) {
