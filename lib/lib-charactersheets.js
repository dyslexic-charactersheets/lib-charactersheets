...
function __(str, doc) {
  ...
  if (!isString(str)) {
    ...
    throw new handlebars.Exception("Not a string");
  }
...
var calc = {
  ...
  render: function render(args, reg, doc) {
    ...
    var outputs = isEmpty(args.outputs) ? [args.output] : args.outputs;
...
var label = {
  ...
  render: function render(args, reg, doc) {
    // log("label", "Args", args);
    var cls = elementClass('label', null, args, ["rotate", "nowrap"], {
      "align": ""
    });
...
var value_block = {
  ...
  transform: function transform(args, ctx) {
    var value = _objectSpread({}, args, {
      type: 'field'
    });

    var contents = isEmpty(args.contents) ? {
      type: 'p',
      content: args.content
    } : {
      type: 'g',
...
function () {
  function Registry() {
    ...
    [unit, // document,
    advancement, article, blockquote, calc, class_name, class_icon, color, each, embed$1, flags, g, h1, h2, h3, h4, h5, h6, hr, tail, vr, icon, ifelem, label, layout, place, indent, level, level_marker, cost, list, join, logelem, logo, lookup, nothing, page, collate_pages, p, ul, li, dl, portrait, proficiency, action, selectable, repeat, row, ruby, ruby_round_up, ruby_round_down, section, sort, slots, spacer, span, spells_list, spells_table, spells_list2, table, copy, paste, unit, zone, field, field_frame_none, field_frame_above, field_frame_left, field_frame_right, field_frame_h_label, field_frame_annotation, field_frame_circle, field_control_input, field_control_value, field_control_ref, field_control_speed, field_control_alignment, field_control_boost, field_control_checkbox, field_control_radio, field_control_checkgrid, field_control_compound, field_control_progression, field_control_money, field_control_weight, field_control_proficiency, field_control_proficiency_icon, field_control_icon, value, value_block].forEach(function (elem) {
  ...
  _createClass(Document, [{
      ...
      while (this.delayedBlocks.length > 0) {
        ...
        blocks.forEach(function (block) {
          ...
          if (has(block, "params") || has(template, "params")) {
            var params = _objectSpread({}, template.params, {}, block.params); // log("Document", "Interpolating parameters", params);
    ...
    value: function getStylesheet() {
      ...
      this.units.forEach(function (unit) {
        ...
        if (unit.id != "document") rendered = replaceColours(rendered, _this9.printColour, _this9.accentColour, _this9.printIntensity, _this9.highContrast);
      ...
      if (this.backgroundURL) {
        ...
        cssParts.push(".page__background{background: ".concat(this.backgroundColour, ";}"));
      } // custom extras


      this.cssParts.forEach(function (css) {
        _this9.cssParts.push(css);
      });
    ...
    value: function getJavascript() {
      ...
      this.jsParts.forEach(function (js) {
        _this10.jsParts.push(processJS(js));
      });
      return jsParts.join("\n");
    }
  }, {
  ...
  switch (attr.game) {
      ...
      if (attr.class) {
        ...
        Object.keys(attr).forEach(function (key) {
          ...
          if (key.startsWith(classPrefix) && !key.endsWith('Feats')) {
            ...
            var selname = key;
            log$1("Character", "Class selector", selname);
            ...
            if (isArray(attr[key])) {
              attr[key].forEach(function (selvalue) {
                ...
                var unitname = selvalue;
                log$1("Character", "Subclass unit name", unitname);
              ...
              // log("Character", "Class option", key, selname, "=", selvalue);
              // const unitname = classShortName + '/' + selname + '/' + selvalue;
              var unitname = attr[key];
              log$1("Character", "Class option unit", unitname);
              char.units.push(unitname);
            }
          }
      ...
      if (attr.archetypes && isArray(attr.archetypes)) {
        attr.archetypes.forEach(function (archetype) {
          if (isString(archetype)) {
            ...
            char.units.push('archetype/' + archetype.replace(/^archetype[\/-]/, '')); // log("Character", "Archetype:", "archetype/"+archetype);
          }
        });
      }
    ...
    case 'starfinder':
      ...
      if (attr.class && isArray(attr.class)) {
        attr.class.forEach(function (cls) {
          if (isString(cls)) {
            char.classes.push(cls);
            char.units.push('class:' + cls.replace(/^class[\/:-]/, '')); // log("Character", "Archetype:", "archetype/"+archetype);
          }
        });
      }
...
function (_Instance) {
  ...
  function Character(primary, request, registry) {
    ...
    _this11 = _possibleConstructorReturn(this, _getPrototypeOf(Character).call(this));
  ...
  _createClass(Character, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        log$1("Character", "Units: ".concat(_this12.data.units));
        ready(function () {
          try {
...
function pathfinder2Title(units, doc, data) {
  ...
  if (!isEmpty(archetypes)) {
    ...
    log$1("Character", "Archetypes:", parts["archetypes"]);
  }
...
function starfinderTitle(units, doc, data) {
  ...
  var classes = getUnits("class");

  if (!isEmpty(classes)) {
    parts["classes"] = classes.map(function (cls) {
      return __(cls.name, doc);
    }).join(" ");
  }
  ...
  var template = isEmpty(parts.name) ? "_{#{race} #{theme} #{classes} #{archetypes}}" : "_{#{name}, #{race} #{theme} #{classes} #{archetypes}}";
  var title = interpolate(__(template, doc), parts);
  title = title.replace(/  +/g, ' ');
  title = title.replace(/^ +/, '');
  title = title.replace(/ +$/, '');
...
function (_Instance2) {
  ...
  function Party(primary, request, registry) {
    ...
    _this13 = _possibleConstructorReturn(this, _getPrototypeOf(Party).call(this));
...
function (_Instance3) {
  ...
  function GM_Instance(request, registry) {
    ...
    _this15 = _possibleConstructorReturn(this, _getPrototypeOf(GM_Instance).call(this));
      ...
      return new Promise(function (resolve, reject) {
        ...
        ready(function () {
          try {
            ...
            if (data.printBackground) {
              var printBackground = data.printBackground; // log("GM", "Background:", printBackground);
            ...
            document.composeDocument(self.registry); // make the element tree
...
function (_GM_Instance) {
  ...
  function GM_Party(primary, request, registry) {
    ...
    _this17 = _possibleConstructorReturn(this, _getPrototypeOf(GM_Party).call(this, request, registry));
  ...
  _createClass(Build, [{
    ...
    value: function render() {
      ...
      return new Promise(function (resolve, reject) {
        ready(function () {
          try {
            log$1("Build", "Data", _this18.data);
            var system = getSystem(_this18.data.game);
            var documentUnit = system.getUnit("document");
            var document = new Document(documentUnit);
            document.language = _this18.data.language;
            var units = system.getUnits(_this18.data.units);
            units = system.inferUnits(units);
            ...
            document.composeDocument(_this18.registry);
            document.title = "Build a character"; // log("Build", "Document", document);
            // render the document

            var data = document.renderDocument(_this18.registry);
            resolve({
              data: data,
  ...
  _createClass(Request, [{
    ...
    value: function parse(request) {
      ...
      if (has(request, "included")) {
        if (isArray(request.included)) {
          request.included.forEach(function (instance) {
            return _this19.addInstance(instance, false);
          });
        }
      ...
      if (isArray(request.data)) {
        request.data.forEach(function (instance) {
          return _this19.addInstance(instance, true);
        });
      } else {
    ...
    value: function getPrimaries(registry) {
        ...
        if (has(primary, "relationships")) {
          Object.keys(primary.relationships).forEach(function (relkey) {
            primary.attributes[relkey] = primary.relationships[relkey].data.flatMap(function (link) {
              if (has(_this20.instances, link.id)) {
                return [_this20.instances[link.id]];
              } else {
                return [];
              }
      ...
      return primaries.map(function (primary) {
        switch (primary.type) {
          case 'character':
            return new Character(primary, _this20, registry);

          case 'party':
            return new Party(primary, _this20, registry);

          case 'gm':
            switch (primary.attributes['gm']) {
              case 'party':
                return new GM_Party(primary, _this20, registry);
            }

          case 'quick':
            switch (primary.attributes['quick']) {
              case 'build':
                return new Build(primary, _this20, registry);
            }
...
loadSystemData(['common', 'pathfinder2', 'starfinder', 'premium']);
