...
var color = require('color'); // Polyfills
...
function embed(contents) {
  ...
  warn$1("util", "Embed: unknown element!", contents);
  return contents;
}
...
function replaceColours(str, documentColour, accentColour, intensity, highContrast) {
    ...
    case 'field':
      {
        ...
        var _rubyHeight = args.ruby.split(/\n/).length;
        return _rubyHeight;
      }
  ...
  return new Promise(function (resolve, reject) {
    if (fs$1.existsSync(filename)) {
      fs$1.readFile(filename, 'utf-8', function (err, data) {
        ...
        if (isDefault) {
          log$1("i18n", "Loaded ".concat(num, " translations for ").concat(lang));
        } else {
          log$1("i18n", "Loaded ".concat(num, " translations for ").concat(lang), filename);
        }
...
var flags = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "<p".concat(cls, ">").concat(_e(label, doc)).concat(reg.render(flags, doc), "</p>");
  }
};
var g = {
  name: 'g',
...
function renderHeading(h) {
  return function (args, reg, doc) {
    ...
    var cls = elementClass(h, null, args, ['fade', 'bold', 'blk'], {
      'align': ''
    });
...
var layout = {
  ...
  render: function render(args, reg, doc) {
    ...
    if (columns > 0 && args.contents.length > columns) {
      log$1("layout", "Split: ".concat(args.contents.length, " items into ").concat(columns, " columns"));
      var items = [];

      for (var n = 0; n < args.contents.length; n += columns) {
        var chunk = args.contents.slice(n, n + columns);
        items.push(_objectSpread({}, args, {
          contents: chunk
        }));
      }

      log$1("layout", "Split render", items);
      return reg.render(items, doc);
    }

    var cls = elementClass('layout', null, args, ['no-flex', 'blk', 'unblk'], {
      'layout': '',
      'gutter': '',
      'flex': false
    });
...
var p = {
  ...
  render: function render(args, reg, doc) {
    var cls = elementClass('p', null, args, ['blk', 'nowrap'], {
      'align': 'left',
      'size': 'medium'
    ...
    if (args.prose) {
      ...
      firstpara = "<p".concat(cls, ">").concat(icon).concat(esc(firstpara), "</p>");
    ...
    return "<p".concat(cls, ">").concat(icon).concat(_e(content, doc), "</p>");
  }
};
var portrait = {
  name: 'portrait',
...
var sort = {
  ...
  transform: function transform(args, ctx) {
    ...
    var contents = args.contents.sort(function (a, b) {
    ...
    if (args.order == 'DESC') {
      contents = (_readOnlyError("contents"), contents.reverse());
    }
...
function renderTableFlipped(args, reg, doc, headings, cols) {
  ...
  var trows = cells.map(function (row) {
    ...
    if (hasHeading) {
      ...
      th = "<th scope=\"row\"".concat(rowspan, ">").concat(isNull(head) ? '' : reg.renderItem(head, doc), "</th>");
    }
...
var copy = {
  ...
  transform: function transform(args, ctx) {
    log$1("template", "Copy template:", args.template);
    ctx.templates[args.template] = {
      params: args.params,
      contents: args.contents
    };
    args.alias.forEach(function (alias) {
      log$1("template", "Copy template alias:", args.template);
      ctx.templates[alias] = {
        params: args.params,
        contents: args.contents
      };
...
function fieldInner(args, reg, doc, decoration) {
  ...
  if (args.repeat > 1) {
    ...
    var _control = reg.renderItem(args, doc);

    if (merge_bottom && boxargs['border'] == 'bottom') boxargs['border'] = 'none';

    var _boxcls = elementClass('field', 'box', boxargs, ["icon"], {
      "border": "bottom"
    });

    inner = "<div".concat(_boxcls, ">").concat(icon).concat(_control).concat(unit, "</div>");
  }
...
function defaultControlRender(args, reg, doc) {
  ...
  var value = args.value == '' ? '' : " value='".concat(__(args.value, doc), "'");
      ...
      var _loop3 = function _loop3(i) {
        ...
        units.forEach(function (unit) {
          if (has(unit, "require") && !isArray(unit.require)) {
            error("Character", "Require not an array in ".concat(unit.id), unit.require);
          }
  ...
  codes.forEach(function (code) {
    ...
    var promise = new Promise(function (resolve, reject) {
      fs$1.readFile(systemFile, 'utf-8', function (err, data) {
        if (err) {
          error("System", "Error loading system file ".concat(systemFile, ":"), err);
          resolve();
          return;
        }
  ...
  _createClass(Document, [{
    ...
    value: function getVar(varname) {
      ...
      this.vars[varname].forEach(function (include) {
        var priority = has(include, "priority") ? include.priority : 'medium';

        switch (priority) {
          case 'high':
            high.push(include.value);
            break;

          case 'medium':
            medium.push(include.value);
            break;

          case 'low':
          default:
            low.push(include.value);
            break;
        }
      });
      var incs = isEmpty(high) ? isEmpty(medium) ? low : medium : high; // TODO type hints
      // TODO combine multiple values somehow

      var is_array = false;
      incs.forEach(function (include) {
        if (isArray(include.value)) is_array = true;
      });
    ...
    value: function addUnit(unit) {
      ...
      if (has(unit, "inc")) {
        unit.inc.forEach(function (include) {
          ...
          switch (directive) {
            case 'at':
              if (has(include, "add")) _this6.addAtZone(include.at, include.add, false);
              if (has(include, "replace")) _this6.addAtZone(include.at, include.replace, true);
              break;
            ...
            case 'set':
              ...
              if (!has(_this6.vars, include.set)) _this6.vars[include.set] = [];

              _this6.vars[include.set].push(include);

              break;

            case 'copy':
              // log("Document", "Copy template:", include.copy);
              _this6.templates[include.copy] = {
    ...
    value: function composeDocument(registry) {
      ...
      function complete(element) {
        if (isArray(element)) return element.flatMap(complete);
        ...
        switch (element.type) {
          ...
          case 'slots':
            ...
            if (reg && reg.transform) {
              ...
              var args = Object.assign({}, reg.defaults, element); //const ctx = { zones: self.zones, templates: self.templates, largePrint: self.largePrint, language: self.language };
      ...
      function compose(element) {
        ...
        ["contents", "placeholder", "header", "inputs"].forEach(function (item_key) {
          ...
          if (has(element, item_key)) {
            ...
            if (isArray(element[item_key])) element[item_key] = element[item_key].flatMap(compose);else element[item_key] = compose(element[item_key]);
          }
...
function locateAsset(filename, cb) {
  ...
  assetsDirs.flatMap(function (dir) {
    ...
    var file = needsBase64(filename) ? "".concat(name, ".base64") : name;
...
var knownVars = ["inventoryStyle", "language", "miniSize"];
...
function parseCharacter(primary, request) {
  ...
  Object.keys(attr).forEach(function (key) {
    ...
    if (flag.match(/^option-/)) {
      ...
      log$1("Character", "Option", option, ok);
  ...
  switch (attr.game) {
    case 'pathfinder2':
      if (attr.ancestry) {
        ...
        if (attr.heritage) char.units.push('heritage/' + char.ancestry + "/" + attr.heritage.replace(/^heritage-/, ''));
      }
      ...
      if (attr.class) {
        ...
        if (attr[classFeatsKey]) {
          ...
          char.classFeats.forEach(function (feat) {
            log$1("Character", "Class feat:", feat);
          ...
          if (key.startsWith(classPrefix) && !key.endsWith('Feats')) {
            ...
            if (isArray(attr[key])) {
              attr[key].forEach(function (selvalue) {
                ...
                log$1("Character", "Class option", key, selname, "=", selvalue);
...
function () {
  ...
  _createClass(Character, [{
    ...
    value: function getAsset(asset, callback) {
      ...
      if (asset === null) {
        ...
        locateAsset(asset, function (assetFile) {
          _this9.loadQueue.loadEmbed(assetFile).then(function (data) {
    ...
    value: function render(callback) {
      ...
      ready(function () {
        try {
          var system = getSystem(_this10.data.game);

          if (system === null) {
            error("Character", "System not found:", _this10.data.game);
            return;
          ...
          document.language = _this10.data.language;
          document.setVar('description', _this10.data.description);

          if (_this10.data.printLarge) {
            document.largePrint = true;
          }

          if (_this10.data.printHighContrast) {
            document.highContrast = true;
          } // Load assets


          if (_this10.data.favicon) {
            _this10.getAsset(_this10.data.favicon, function (dataURL) {
              document.faviconURL = dataURL;
            });
          }

          if (_this10.data.printLogo) {
            _this10.getAsset(_this10.data.printLogo, function (dataURL) {
              document.logoURL = dataURL;
            });
          }

          if (_this10.data.printPortrait) {
            _this10.getAsset(_this10.data.printPortrait, function (dataURL) {
              document.portraitURL = dataURL;
            });
          }

          if (_this10.data.animalPortrait) {
            _this10.getAsset(_this10.data.animalPortrait, function (dataURL) {
              document.animalURL = dataURL;
            });
          }

          if (_this10.data.printBackground) {
            var printBackground = _this10.data.printBackground; // log("Character", "Background:", printBackground);
            ...
            if (has(bgColours, printBackground)) {
              ...
              _this10.getAsset(printBackground, function (dataURL) {
          ...
          document.printColour = _this10.data.printColour;
          document.printIntensity = _this10.data.printIntensity;
          document.accentColour = _this10.data.accentColour;
          document.watermark = _this10.data.printWatermark; // get known vars from the data

          knownVars.forEach(function (varname) {
            if (has(_this10.data, varname)) {
              var key = toKebabCase(varname);
              var value = _this10.data[varname]; // log("Character", `Var: ${key} = ${JSON.stringify(value)}`);

              document.setVar(key, value, "high");
            }
          }); // load units

          var units = system.getUnits(_this10.data.units);
          units = system.inferUnits(units);
          log$1("Character", "Units:", units.map(function (unit) {
            return unit.id;
          }).sort()); // infer the title from the units

          var title = __("Character");

          switch (system.code) {
            case 'pathfinder2':
              title = _this10.pathfinder2Title(units, document);
              break;
          }
          ...
          document.composeDocument(_this10.registry);

          _this10.loadQueue.ready(function () {
            Events.createElementTreeEvt.call(document.doc, document.title, _this10.request); // render the document

            var data = document.renderDocument(_this10.registry);
            callback({
              data: data,
              filename: title + ".html",
              mimeType: "text/html"
    ...
    key: "pathfinder2Title",
    value: function pathfinder2Title(units, doc) {
      var parts = {
        name: this.data.name,
        ancestry: '',
        class: '',
...
function () {
  ...
  _createClass(Party, [{
    ...
    value: function render(callback) {
      ...
      Promise.all(promises).then(function () {
        log$1("Party", "Rendered ".concat(files.length, " members"));
        callback(files);
      });
    }
  }]);
...
function () {
  ...
  _createClass(Build, [{
    ...
    value: function render(callback) {
      ...
      ready(function () {
        try {
          log$1("Build", "Data", _this11.data);
          var system = getSystem(_this11.data.game);
          var documentUnit = system.getUnit("document");
          var document = new Document(documentUnit);
          document.language = _this11.data.language;
          var units = system.getUnits(_this11.data.units);
          units = system.inferUnits(units);
          log$1("Build", "Units", units.map(function (unit) {
            return unit.name;
          }));
          units.forEach(function (unit) {
            return document.addUnit(unit);
          });
          document.composeDocument(_this11.registry);
          document.title = "Build a character"; // log("Build", "Document", document);
          // render the document

          var data = document.renderDocument(_this11.registry);
          callback({
            data: data,
  ...
  _createClass(Request, [{
    ...
    value: function parse(request) {
      ...
      if (has(request, "included")) {
        if (isArray(request.included)) {
          request.included.forEach(function (instance) {
            return _this12.addInstance(instance, false);
          });
        }
      ...
      if (isArray(request.data)) {
        request.data.forEach(function (instance) {
          return _this12.addInstance(instance, true);
        });
      } else {
    ...
    value: function getPrimaries(registry) {
        ...
        if (has(primary, "relationships")) {
          Object.keys(primary.relationships).forEach(function (relkey) {
            primary.attributes[relkey] = primary.relationships[relkey].data.flatMap(function (link) {
              if (has(_this13.instances, link.id)) {
                return [_this13.instances[link.id]];
              } else {
                return [];
              }
      ...
      return primaries.map(function (primary) {
        switch (primary.type) {
          case 'character':
            return new Character(primary, _this13, registry);

          case 'party':
            return new Party(primary, _this13, registry);

          case 'quick':
            switch (primary.attributes['quick']) {
              case 'build':
                return new Build(primary, _this13, registry);
            }
...
function getFormData(system, callback) {
  ready(function () {
    if (!has(promises$1, system)) {
      promises$1[system] = new Promise(function (resolve, reject) {
        var formDataFile = "".concat(__dirname, "/data-").concat(system, ".json");
        log$1('formdata', 'Loading file', formDataFile);
        fs$1.readFile(formDataFile, 'utf-8', function (err, data) {
          if (err) {
            error('formdata', "Error loading form data ".concat(formDataFile, ":"), err);
            reject();
            return;
          }

          log$1('formdata', 'Loaded file', formDataFile);
    ...
    promises$1[system].then(callback);
  });
...
var addTranslator$1 = addTranslator; // export function loadPo(lang, filename = null) {
//   loadPoF(lang, filename);
// }

var addTranslationData$1 = addTranslationData;
var loadTranslations$1 = loadTranslations;
var loadDefaultTranslations$1 = loadDefaultTranslations;
...
var getFormData$1 = getFormData;
exports.addAssetsDir = addAssetsDir$1;
exports.addTranslationData = addTranslationData$1;
exports.addTranslator = addTranslator$1;
exports.create = create;
exports.getFormData = getFormData$1;
exports.loadDefaultTranslations = loadDefaultTranslations$1;
exports.loadTranslations = loadTranslations$1;
