...
function elementClass(block) {
  ...
  Object.entries(attribs).forEach(function (pair) {
    var key = pair[0];
    var value = pair[1];
    if (isEmpty(value)) return; // console.log("  -", key, " = ", value);
...
function replaceColours(str, documentColour, accentColour, highContrast) {
  ...
  str = str.replace(/rgba\(.*?,.*?,.*?,(.*?)\)/g, function (c) {
    return adjustColour(c, documentColour, highContrast);
  }); // (c, opacity) => adjustColourRGBA(c, opacity, highContrast));
...
function adjustColour(c, documentColour, highContrast) {
  try {
    ...
    if (alpha != 1) {
      ...
      var _result = "rgba(".concat(red, ",").concat(green, ",").concat(blue, ",").concat(alpha, ")"); // console.log(" Alpha colour:", result);
    ...
    var _loop = function _loop(_lv) {
      ...
      itemsByLevel[_lv].forEach(function (item) {
        ...
        if (has(item, "advance")) {
          ...
          has_labels = true; // } else if (has(item, "proficiency")) {
      ...
      delete flags.type; // delete flags.proficiency;
      // delete flags.in;
...
function __(str, doc) {
  ...
  return str.replace(/_\{([\0-\uFFFF]*?)\}/g, function (m, p) {
    return translate(p);
  });
}
...
var each = {
  ...
  transform: function transform(args) {
    ...
    return args.contents.flatMap(function (item) {
      ...
      if (i < args.rows.length && isObject(args.rows[i])) values = Object.assign(args.rows[i], values);
      values['item'] = item;
...
var list = {
  ...
  render: function render(args, reg, doc) {
    ...
    var cls = elementClass('list', null, args, ["zebra", "zebra-inverse", "collapse", "vr", "hr", "merge-bottom"], {
      "flex": false
    });
    return "<div".concat(cls, ">").concat(reg.render(args.contents, doc), "</div>");
  },
...
var repeat = {
  ...
  transform: function transform(args, ctx) {
    var contents = [];
    if (ctx.largePrint && args.reduce > 0) args.repeat -= args.reduce;
...
function renderTableFlipped(args, reg, doc, headings, cols) {
  ...
  if (hasHeading) _readOnlyError("ncols"), ncols++; // log("table", `Table: ${nrows} rows, ${ncols} cols`);
...
function defaultControlRender(args, reg, doc) {
  ...
  var overlay = args.overlay ? "<span class='field__overlay'>".concat(__(args.overlay), "</span>") : '';
  return "".concat(prefix, "<div").concat(cls, ">").concat(input).concat(underlay, "</div>").concat(overlay);
}
...
var field_control_value = {
  ...
  render: function render(args, reg, doc) {
    ...
    return "".concat(prefix, "<div").concat(cls, ">").concat(value).concat(underlay, "</div>").concat(overlay);
  }
};
...
var field_control_money = {
  ...
  render: function render(args, reg, doc) {
    ...
    var overlay = "<span class='field__overlay'>".concat(__(unit), "</span>");
    var ident = fieldIdent(args.id);
    var cls = elementClass("field", "control", args, [], {
      "digits": 0,
      "shift": 0,
...
function parseCharacter(primary, request) {
  ...
  Object.keys(attr).forEach(function (key) {
    if (key.match(/^option/)) {
      var flag = toKebabCase(key.replace(/^option/, ''));
...
function getFormData(system, callback) {
  if (!has(promises$1, system)) {
    promises$1[system] = new Promise(function (resolve, reject) {
      var formDataFile = __dirname + '/data-' + system + '.json';
